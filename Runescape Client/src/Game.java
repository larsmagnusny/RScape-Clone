...
public class Game extends RSApplet
{
	...
	// check if music is null, if not call a function
	static final void setMusicInstanceToNull()
	{
		if (musicIsntNull())
		{
			method891(false);
		    ...
		    music.closeSequencerAndReciever();
		    music = null;
		}
	}
	...
	public void repackCacheIndex(int cacheIndex)
	{
		...
		try
		{
			for (int index = 0; index < indexLength; index++)
			{
				...
				{
					decompressors[cacheIndex].decompressSound(data.length, data, fileIndex);
	...
	public void decompressAllSounds()
	{
		for(int MusicIndex = 0; MusicIndex < 3536; MusicIndex++) 
		{
			byte[] soundBuffer = getRawSound(MusicIndex);
			if (soundBuffer != null && soundBuffer.length > 0) {
				decompressors[3].decompressSound(soundBuffer.length, soundBuffer, MusicIndex);
			}
		}
	}
	
	public byte[] getRawSound(int Index) {
		try {
		File sound = new File(Signlink.findcachedir() + "./sounds/"+Index+".gz");
		byte[] soundBuffer = new byte[(int)sound.length()];
		FileInputStream Fis = new FileInputStream(sound);
		Fis.read(soundBuffer);
		Fis.close();
		return soundBuffer;
		} catch(Exception e) {
		return null;
	...
	static final boolean constructMusic() {
		anInt720 = 20;
		try {
		    music = (SoundClass) Class.forName("MidiClass").newInstance();
	...
	final synchronized void setNextSong(int i_30_, int volume,
		    boolean bool, int music)
	{
		if (musicIsntNull()) 
		{
			nextSong = music;
			onDemandFetcher.getSongFromIndex(2, nextSong);
			musicVolume2 = volume;
			anInt139 = -1;
			aBoolean995 = true;
			anInt116 = i_30_;
	...
	final synchronized void setNextSong2(int volume, boolean bool, int music)
	{
		if (musicIsntNull())
		{
			nextSong = music;
	...
	static final synchronized void method49()
	{
		if (musicIsntNull()) 
		{
			if (fetchMusic) 
			{
				byte[] buffer = musicData;
				if (buffer != null) 
				{
					if (anInt116 >= 0)
						method684(aBoolean995, anInt116, musicVolume2, buffer);
					else if (anInt139 >= 0)
						method899(anInt139, -1, aBoolean995, buffer, musicVolume2);
					else
						method853(musicVolume2, buffer, aBoolean995);
					fetchMusic = false;
				}
			}
		    method368(0);
	...
	static final void method853(int i_2_, byte[] soundData, boolean bool)
	{
		if (music != null)
		{
			if (anInt478 >= 0)
		    ...
		    if (soundData != null)
		    {
		    	if (anInt720 > 0)
		    	{
		    		music.method831(i_2_);
		    		anInt720 = 0;
		    	}
		    	anInt478 = i_2_;
		    	music.playMidiSequence(i_2_, soundData, 0, bool);
		    }
		}
	}
	...
	static final void method368(int i)
	{
		if (music != null)
		{
			if (anInt478 < i)
			{
				if (anInt720 > 0)
				{
					...
					if (anInt720 == 0)
					{
						...
						{
							music.method831(anInt1478);
							anInt478 = anInt1478;
							music.playMidiSequence(anInt1478, aByteArray347, 0, aBoolean475);
							aByteArray347 = null;
						}
						anInt155 = 0;
					}
			...
			{
				anInt155 += anInt2200;
				music.method830(anInt478, anInt155);
				anInt720--;
				if (anInt720 == 0)
				{
					music.stopMusic();
	...
	public void drawChatArea()
	{
		...
		{
			chatTextDrawingArea.drawText(0, friendAddString, 40, 239);
		...
		{
			TextDrawingArea chatDrawingArea = mChatDrawingArea;
			int j = 0;
			...
			for (int k = 0; k < 100; k++) 
			{
				if (chatMessages[k] != null) 
				{
					...
					if (chatType == 0)
					{
						if (i1 > 0 && i1 < 110) 
						{
							chatDrawingArea.method389(false, 4, 0, chatMessages[k], i1);
						}
						j++;
					}
					if ((chatType == 1 || chatType == 2) && (chatType == 1 || publicChatMode == 0 || publicChatMode == 1 && isFriendOrSelf(chatName)))
					{
						if (i1 > 0 && i1 < 110) 
						{
							int j1 = 4;
							...
							chatDrawingArea.drawTextMessage(0, chatName + ":", i1, j1);
							j1 += chatDrawingArea.getTextWidth(chatName) + 8;
							chatDrawingArea.drawTextMessage(255, chatMessages[k], i1, j1);
						}
						j++;
					}
					if ((chatType == 3 || chatType == 7) && splitpublicChat == 0 && (chatType == 7 || privateChatMode == 0 || privateChatMode == 1 && isFriendOrSelf(chatName))) {
						if (i1 > 0 && i1 < 110) 
						{
							int k1 = 4;
							chatDrawingArea.drawTextMessage(0, "From", i1, k1);
							k1 += chatDrawingArea.getTextWidth("From ");
							if (byte0 == 1)
							{
							...
							chatDrawingArea.drawTextMessage(0, chatName + ":", i1, k1);
							k1 += chatDrawingArea.getTextWidth(chatName) + 8;
							chatDrawingArea.drawTextMessage(0x800000, chatMessages[k], i1, k1);
						}
						j++;
					}
					if (chatType == 4 && (tradeMode == 0 || tradeMode == 1 && isFriendOrSelf(chatName)))
					{
						if (i1 > 0 && i1 < 110) 
						{
							chatDrawingArea.drawTextMessage(0x800080, chatName + " " + chatMessages[k], i1, 4);
						}
						j++;
					}
					if (chatType == 5 && splitpublicChat == 0 && privateChatMode < 2) 
					{
						if (i1 > 0 && i1 < 110) 
						{
							chatDrawingArea.drawTextMessage(0x800000, chatMessages[k], i1, 4);
						}
						j++;
					}
					if (chatType == 6 && splitpublicChat == 0 && privateChatMode < 2) 
					{
						if (i1 > 0 && i1 < 110) 
						{
							chatDrawingArea.drawTextMessage(0, "To " + chatName + ":", i1, 4);
							chatDrawingArea.drawTextMessage(0x800000, chatMessages[k], i1, 12 + chatDrawingArea.getTextWidth("To " + chatName));
						}
						j++;
					}
					if (chatType == 8 && (tradeMode == 0 || tradeMode == 1 && isFriendOrSelf(chatName))) 
					{
						if (i1 > 0 && i1 < 110) 
						{
							chatDrawingArea.drawTextMessage(0x7e3200, chatName + " " + chatMessages[k], i1, 4);
						}
						j++;
					}
				}
			...
			chatDrawingArea.drawTextMessage(0, playerName + ":", 90, 4);
			chatDrawingArea.drawTextMessage(255, inputString + "*", 90, 6 + chatDrawingArea.getTextWidth(playerName + ": "));
		...
		chatImageProducer.drawGraphics(357, super.graphics, 17);
	...
	public void startRunnableThread(Runnable runnable, int i) {
		if (i > 10)
		{
			i = 10;
		}
		...
		{
			super.startRunnableThread(runnable, i);
		}
	}
	...
	public void processMenuClick() 
	{
		...
		if (menuOpen)
		{
			if (clickMode != 1) 
			{
				...
				if (mouseX < mMenuOffsetX - 10 || 
					mouseX > mMenuOffsetX + mMenuWidth + 10 ||
					mouseY < mMenuOffsetY - 10 ||
					mouseY > mMenuOffsetY + mMenuHeight + 10)
				{
					menuOpen = false;
					if (menuScreenArea == 1)
					{
			...
			if (clickMode == 1) 
			{
				int offsetX = mMenuOffsetX;
				int offsetY = mMenuOffsetY;
				int width = mMenuWidth;
				int sClickX = super.saveClickX;
				int sClickY = super.saveClickY;
				if (menuScreenArea == 0)
				{
					sClickX -= 4;
					sClickY -= 4;
				...
				{
					int k3 = offsetY + 31 + (menuActionRow - 1 - i) * 15;
					if (sClickX > offsetX && sClickX < offsetX + width && sClickY > k3 - 13 && sClickY < k3 + 3)
					{
						action = i;
					}
				}
		...
		else 
		{
			...
			{
				int menuAction = menuActionIDs[menuActionRow - 1];
				if (menuAction == 632 || menuAction == 78 || menuAction == 867 || menuAction == 431 || menuAction == 53 || menuAction == 74 || menuAction == 454 || menuAction == 539 || menuAction == 493 || menuAction == 847 || menuAction == 447 || menuAction == 1125)
				{
					int menuAction2 = menuInventorySlots[menuActionRow - 1];
					int menuAction3 = menuInvInterfaceIDs[menuActionRow - 1];
					RSInterface actionInterface = RSInterface.interfaceCache[menuAction3];
					if (actionInterface.aBoolean259 || actionInterface.aBoolean235)
					{
						aBoolean1242 = false;
						anInt989 = 0;
						lastMenuAction3 = menuAction3;
						mLastMenuAction2 = menuAction2;
						activeInterfaceType = 2;
						lastSaveClickX = super.saveClickX;
						lastSaveClickY = super.saveClickY;
						if (RSInterface.interfaceCache[menuAction3].parentID == openInterfaceID)
						{
							activeInterfaceType = 1;
						}
						if (RSInterface.interfaceCache[menuAction3].parentID == backDialogID) 
						{
	...
	public void saveMidi(boolean midiFade, byte rawData[]) 
	{
		Signlink.midifade = midiFade ? 1 : 0;
		Signlink.saveMidi(rawData, rawData.length);
	}
	...
	public void method22() 
	{
		...
		{
			anInt985 = -1;
			mNodeList1.removeAll();
			mNodeList2.removeAll();
			Texture.method366();
			unlinkMRUNodes();
			...
			for (int i = 0; i < 4; i++)
			{
				mCollisionMap[i].resetCollisionMap();
			}
			...
			int byteArrayLength = aByteArrayArray1183.length;
			mStream.createFrame(0);
			if (!aBoolean1159)
			{
				for (int i3 = 0; i3 < byteArrayLength; i3++) 
				{
					...
					if (abyte0 != null) 
					{
						objectManager.method180(abyte0, k5, i4, (anInt1069 - 6) * 8, (anInt1070 - 6) * 8, mCollisionMap);
					}
				}

				for (int j4 = 0; j4 < byteArrayLength; j4++)
				{
				...
				{
					anInt1097 = 0;
					mStream.createFrame(238);
					mStream.writeWordBigEndian(96);
				}
				mStream.createFrame(0);
				for (int i6 = 0; i6 < byteArrayLength; i6++)
				{
					...
					if (abyte1 != null)
					{
						...
						objectManager.method190(l8, mCollisionMap, k9, worldController, abyte1);
					}
				}
			}
			if (aBoolean1159)
						...
						for (int j6 = 0; j6 < 13; j6++)
						{
							...
							if (l7 != -1) 
							{
								...
								for (int l11 = 0; l11 < anIntArray1234.length; l11++) 
								{
									...
									objectManager.method179(i9, l9, mCollisionMap, k4 * 8, (j10 & 7) * 8, aByteArrayArray1183[l11], (l10 & 7) * 8, j3, j6 * 8);
									break;
								}
							}
						}
						...
						for (int j9 = 0; j9 < 13; j9++)
						{
							...
							if (i10 != -1) 
							{
								...
								for (int k12 = 0; k12 < anIntArray1234.length; k12++) 
								{
									...
									objectManager.method183(mCollisionMap, worldController, k10, j8 * 8, (i12 & 7) * 8, l6, aByteArrayArray1247[k12], (k11 & 7) * 8, i11, j9 * 8);
									break;
								}
			...
			mStream.createFrame(0);
			objectManager.method171(mCollisionMap, worldController);
		...
		{
			mStream.createFrame(210);
			mStream.writeDWord(0x3f008edd);
		}
		if (mLowMem && Signlink.cache_dat != null) 
		{
	...
	{
		int spritePixels[] = mSomekindOfSprite.pixels;
		int numSpritePixels = spritePixels.length;
		for (int k = 0; k < numSpritePixels; k++) 
		{
			spritePixels[k] = 0;
		}
		...
		for (int l = 1; l < 103; l++) 
		{
			...
			for (int k1 = 1; k1 < 103; k1++) 
			{
				...
				{
					worldController.method309(spritePixels, i1, i, k1, l);
				}
				if (i < 3 && (byteGroundArray[i + 1][k1][l] & 8) != 0) 
				{
					worldController.method309(spritePixels, i1, i + 1, k1, l);
				}
				i1 += 4;
			}
		}

		int randomizedInt1 = (238 + (int) (Math.random() * 20D) - 10 << 16) + (238 + (int) (Math.random() * 20D) - 10 << 8) + 238 + (int) (Math.random() * 20D) - 10;
		int randomizedInt2 = 238 + (int) (Math.random() * 20D) - 10 << 16;
		mSomekindOfSprite.method343();
		for (int i2 = 1; i2 < 103; i2++) 
		{
			for (int j2 = 1; j2 < 103; j2++) 
			{
				...
				{
					method50(i2, randomizedInt1, j2, randomizedInt2, i);
				}
				if (i < 3 && (byteGroundArray[i + 1][j2][i2] & 8) != 0) 
				{
					method50(i2, randomizedInt1, j2, randomizedInt2, i + 1);
				}
			}
				...
				if (i3 != 0) 
				{
					...
					if (j3 >= 0)
					{
						...
						if (j3 != 22 && j3 != 29 && j3 != 34 && j3 != 36 && j3 != 46 && j3 != 47 && j3 != 48)
						{
							byte byte0 = 104;
							byte byte1 = 104;
							int ai1[][] = mCollisionMap[plane].mCollisionMapData;
	...
	public void spawnGroundItem(int y, int x) 
	{
		NodeList groundNode = groundArray[plane][y][x];
		if (groundNode == null)
		{
			worldController.method295(plane, y, x);
			return;
		}
		int limit = 0xfa0a1f01;
		Object obj = null;
		for (Item item = (Item) groundNode.reverseGetFirst(); item != null; item = (Item) groundNode.reverseGetNext())
		{
			ItemDef itemDef = ItemDef.forID(item.ID);
			int itemDefValue = itemDef.value;
			if (itemDef.stackable) 
			{
				itemDefValue *= item.anInt1559 + 1;
				// notifyItemSpawn(item, i + baseX, j + baseY);
			}

			if (itemDefValue > limit)
			{
				limit = itemDefValue;
				obj = item;
			}
		...
		groundNode.insertTail(((Node) obj));
		Object obj1 = null;
		Object obj2 = null;
		for (Item curItem = (Item) groundNode.reverseGetFirst(); curItem != null; curItem = (Item) groundNode.reverseGetNext()) 
		{
			if (curItem.ID != ((Item) obj).ID && obj1 == null)
			{
				obj1 = curItem;
			}
			if (curItem.ID != ((Item) obj).ID && curItem.ID != ((Item) obj1).ID && obj2 == null)
			{
				obj2 = curItem;
			}
		}

		int i1 = y + (x << 7) + 0x60000000;
		worldController.method281(y, i1, ((Animable) obj1), method42(plane, x * 128 + 64, y * 128 + 64), ((Animable) obj2), ((Animable) obj), plane, x);
	}

	public void method26(boolean flag)
	{
		for (int j = 0; j < mNumNpcs; j++)
		{
			NPC curNpc = npcArray[mNpcIndices[j]];
			int k = 0x20000000 + (mNpcIndices[j] << 14);
			if (curNpc == null || !curNpc.isVisible() || curNpc.desc.aBoolean93 != flag) 
			{
				continue;
			}
			int l = curNpc.x >> 7;
			...
			if (curNpc.anInt1540 == 1 && (curNpc.x & 0x7f) == 64 && (curNpc.y & 0x7f) == 64) 
			{
			...
			if (!curNpc.desc.aBoolean84) 
			{
				k += 0x80000000;
			}
			worldController.method285(plane, curNpc.anInt1552, method42(plane, curNpc.y, curNpc.x), k, curNpc.y, (curNpc.anInt1540 - 1) * 64 + 60, curNpc.x, curNpc, curNpc.aBoolean1541);
		}
	}
	...
	public void buildInterfaceMenu(int i, RSInterface interfaceMenu, int k, int l, int i1, int j1) 
	{
		if (interfaceMenu.type != 0 || interfaceMenu.children == null || interfaceMenu.aBoolean266)
		{
			return;
		}
		if (k < i || i1 < l || k > i + interfaceMenu.width || i1 > l + interfaceMenu.height)
		{
			return;
		}
		int numChildren = interfaceMenu.children.length;
		for (int childIndex = 0; childIndex < numChildren; childIndex++)
		{
			int i2 = interfaceMenu.childX[childIndex] + i;
			int j2 = interfaceMenu.childY[childIndex] + l - j1;
			RSInterface childInterface = RSInterface.interfaceCache[interfaceMenu.children[childIndex]];
			...
			else 
			{
				...
				{
					boolean flag = false;
					if (childInterface.actionID != 0)
					{
						flag = buildFriendsListMenu(childInterface);
					}
					if (!flag) 
					{
						menuActionNames[menuActionRow] = showInfo ? childInterface.tooltip + ", " + childInterface.id : childInterface.tooltip;
						menuActionIDs[menuActionRow] = 315;
						menuInvInterfaceIDs[menuActionRow] = childInterface.id;
						menuActionRow++;
					}
				}
				if (childInterface.atActionType == 2 && spellSelected == 0 && k >= i2 && i1 >= j2 && k < i2 + childInterface.width && i1 < j2 + childInterface.height) 
				{
					...
					menuActionNames[menuActionRow] = actionName + " @gre@" + childInterface.spellName;
				...
				if (childInterface.type == 2)
				{
					...
					for (int l2 = 0; l2 < childInterface.height; l2++)
					{
						for (int i3 = 0; i3 < childInterface.width; i3++) 
						{
							...
							if (inventorySlot < 20) {
								j3 += childInterface.spritesX[inventorySlot];
								k3 += childInterface.spritesY[inventorySlot];
							}
							if (k >= j3 && i1 >= k3 && k < j3 + 32 && i1 < k3 + 32)
							{
								mMouseInvInterfaceIndex = inventorySlot;
								lastActiveInvInterface = childInterface.id;
								if (childInterface.inv[inventorySlot] > 0) 
								{
									ItemDef itemDef = ItemDef.forID(childInterface.inv[inventorySlot] - 1);
									if (itemSelected == 1 && childInterface.isInventoryInterface) 
									{
										if (childInterface.id != anInt1284 || inventorySlot != anInt1283) 
										{
											menuActionNames[menuActionRow] = "Use " + selectedItemName + " with @lre@" + itemDef.name;
											menuActionIDs[menuActionRow] = 870;
											menuItemDefIDs[menuActionRow] = itemDef.id;
											menuInventorySlots[menuActionRow] = inventorySlot;
											menuInvInterfaceIDs[menuActionRow] = childInterface.id;
											menuActionRow++;
										}
									} 
									else if (spellSelected == 1 && childInterface.isInventoryInterface) 
									{
										...
										{
											menuActionNames[menuActionRow] = spellTooltip + " @lre@" + itemDef.name;
									...
									else 
									{
										if (childInterface.isInventoryInterface) 
										{
											for (int l3 = 4; l3 >= 3; l3--) 
											{
												...
												{
													menuActionNames[menuActionRow] = itemDef.actions[l3] + " @lre@" + itemDef.name;
													if (l3 == 3) 
													{
												...
												{
													menuActionNames[menuActionRow] = "Drop @lre@" + itemDef.name;
													menuActionIDs[menuActionRow] = 847;
													menuItemDefIDs[menuActionRow] = itemDef.id;
													menuInventorySlots[menuActionRow] = inventorySlot;
													menuInvInterfaceIDs[menuActionRow] = childInterface.id;
										...
										{
											menuActionNames[menuActionRow] = "Use @lre@" + itemDef.name;
										...
										if (childInterface.isInventoryInterface && itemDef.actions != null) 
										{
											for (int i4 = 2; i4 >= 0; i4--) 
											{
												...
												{
													menuActionNames[menuActionRow] = itemDef.actions[i4] + " @lre@" + itemDef.name;
													if (i4 == 0) 
													{
										...
										if (childInterface.actions != null) 
										{
											for (int j4 = 4; j4 >= 0; j4--) 
											{
												...
												{
													menuActionNames[menuActionRow] = childInterface.actions[j4] + " @lre@" + itemDef.name;
													if (j4 == 0) 
													{
										...
										menuActionNames[menuActionRow] = "Examine @lre@" + itemDef.name + (showInfo ? " @gre@(@whi@" + (childInterface.inv[inventorySlot] - 1) + "@gre@)" : "");
	...
	public void method30(int j, int k, int l, int i1, int j1)
	{
		...
		DrawingArea.method336(j - 32, l + 16, anInt1002, 16, i1);
		int k1 = (j - 32) * j / j1;
		if (k1 < 8) {
			k1 = 8;
		}
		int l1 = (j - 32 - k1) * k / (j1 - j);
		DrawingArea.method336(k1, l + 16 + l1, anInt1063, 16, i1);
	...
	public void updateNPCs(Stream stream, int pSize) 
	{
		anInt839 = 0;
		anInt893 = 0;
		method139(stream);
		method46(pSize, stream);
		method86(stream);
		for (int i = 0; i < anInt839; i++) 
		{
			int npcIndex = anIntArray840[i];
			if (npcArray[npcIndex].loopCycle != loopCycle) 
			{
				npcArray[npcIndex].desc = null;
				npcArray[npcIndex] = null;
			}
		}

		if (stream.currentOffset != pSize)
		{
			Signlink.reporterror(myUsername + " size mismatch in getnpcpos - pos:" + stream.currentOffset + " psize:" + pSize);
			throw new RuntimeException("eek");
		}
		for (int i = 0; i < mNumNpcs; i++)
		{
			if (npcArray[mNpcIndices[i]] == null) {
				Signlink.reporterror(myUsername + " null entry in npc list - pos:" + i + " size:" + mNumNpcs);
				throw new RuntimeException("eek");
			}
		}
	}
	...
	public void processChatModeClick() 
	{
		if (super.clickMode3 == 1) 
		{
			if (super.saveClickX >= 6 && super.saveClickX <= 106 && super.saveClickY >= 467 && super.saveClickY <= 499) 
			{
				...
				aBoolean1233 = true;
				inputTaken = true;
				mStream.createFrame(95);
				mStream.writeWordBigEndian(publicChatMode);
				//stream.writeWordBigEndian(publicChatMode);
				mStream.writeWordBigEndian(privateChatMode);
				mStream.writeWordBigEndian(tradeMode);
			}
			...
			if (super.saveClickX >= 412 && super.saveClickX <= 512 && super.saveClickY >= 467 && super.saveClickY <= 499)
			{
				if (openInterfaceID == -1) 
				{
					...
					{
						if (element == null || element.actionID != 600) 
						{
							continue;
						}
			...
			{
				anInt940 = 0;
				mStream.createFrame(165);
				mStream.writeWordBigEndian(0);
				int j = mStream.currentOffset;
				mStream.writeWordBigEndian(139);
				mStream.writeWordBigEndian(150);
				mStream.writeWord(32131);
				mStream.writeWordBigEndian((int) (Math.random() * 256D));
				mStream.writeWord(3250);
				mStream.writeWordBigEndian(177);
				mStream.writeWord(24859);
				mStream.writeWordBigEndian(119);
				if ((int) (Math.random() * 2D) == 0) 
				{
					mStream.writeWord(47234);
				}
				if ((int) (Math.random() * 2D) == 0)
				{
					mStream.writeWordBigEndian(21);
				}
				mStream.writeBytes(mStream.currentOffset - j);
			}
		}
	}
	...
	public void method33(int i) 
	{
		...
		if (action == 3) 
		{
			...
			if (volume != musicVolume)
			{
				...
				{
					setNextSong2(volume, false, currentSong);
	...
	public void updateEntities() 
	{
		try {
			int anInt974 = 0;
			for (int i = -1; i < mPlayerCount + mNumNpcs; i++) 
			{
				Object obj;
				if (i == -1) 
				{
				...
				{
					obj = mPlayerArray[mPlayerIndices[i]];
				} 
				else 
				{
					obj = npcArray[mNpcIndices[i - mPlayerCount]];
				}
				...
				if (i < mPlayerCount) 
				{
					...
					if (i >= 0 && anInt855 == 10 && anInt933 == mPlayerIndices[i])
					{
						npcScreenPos(((Entity) obj), ((Entity) obj).height + 15);
						if (spriteDrawX > -1) 
						{
				...
				else
				{
					...
					if (anInt855 == 1 && anInt1222 == mNpcIndices[i - mPlayerCount] && loopCycle % 20 < 10)
					{
						npcScreenPos(((Entity) obj), ((Entity) obj).height + 15);
						if (spriteDrawX > -1) 
						{
				...
				if (((Entity) obj).textSpoken != null && (i >= mPlayerCount || publicChatMode == 0 || publicChatMode == 3 || publicChatMode == 1 && isFriendOrSelf(((Player) obj).name)))
				{
			...
			for (int k = 0; k < anInt974; k++) 
			{
				...
				if (anInt1249 == 0)
				{
					...
					if (anIntArray981[k] == 4) 
					{
						...
						chatTextDrawingArea.drawTextMessage(0, s, spriteDrawY + 1, spriteDrawX + 50 - k4);
						chatTextDrawingArea.drawTextMessage(i3, s, spriteDrawY, spriteDrawX + 50 - k4);
						DrawingArea.defaultDrawingAreaSize();
					}
	...
	public void delFriend(long playerID)
	{
		try 
		{
			if (playerID == 0L)
			...
			{
				if (mFriendsListAsLongs[i] != playerID) 
				{
					continue;
				}
				mFriendsCount--;
				mNeedDrawTabArea = true;
				for (int j = i; j < mFriendsCount; j++)
				{
					mFriendsList[j] = mFriendsList[j + 1];
					mFriendsNodeIDs[j] = mFriendsNodeIDs[j + 1];
					mFriendsListAsLongs[j] = mFriendsListAsLongs[j + 1];
				}

				mStream.createFrame(215);
				mStream.writeQWord(playerID);
				break;
			}
		} 
		catch (RuntimeException runtimeexception)
		{
			Signlink.reporterror("18622, " + false + ", " + playerID + ", " + runtimeexception.toString());
	...
	{
		tabAreaImageProducer.initDrawingArea();
		...
		tabAreaImageProducer.drawGraphics(205, super.graphics, 553);
		aRSImageProducer_1165.initDrawingArea();
		Texture.lineOffsets = chatBoxAreaOffsets;
	}
	...
	public void cycleAllText()
	{
		for (int i = -1; i < mPlayerCount; i++) 
		{
			...
			if (i == -1)
			{
				currentPlayerIndex = mMyPlayerIndex;
			} 
			else 
			{
				currentPlayerIndex = mPlayerIndices[i];
			}
			Player player = mPlayerArray[currentPlayerIndex];
	...
	{
		int menuOffsetX = mMenuOffsetX;
		int menuOffsetY = mMenuOffsetY;
		int menuWidth = mMenuWidth;
		int menuHeight = mMenuHeight;
		int i1 = 0x5d5447;
		DrawingArea.method336(menuHeight, menuOffsetY, i1, menuWidth, menuOffsetX);
		DrawingArea.method336(16, menuOffsetY + 1, 0, menuWidth - 2, menuOffsetX + 1);
		DrawingArea.fillPixels(menuOffsetY + 18, menuHeight - 19, 0, menuOffsetX + 1, menuWidth - 2);
		chatTextDrawingArea.drawTextMessage(i1, "Choose Option", menuOffsetY + 14, menuOffsetX + 3);
		int mouseX = super.mouseX;
		int mouseY = super.mouseY;
		if (menuScreenArea == 0) 
		{
		...
		for (int curActionIndex = 0; curActionIndex < menuActionRow; curActionIndex++) 
		{
			int i2 = menuOffsetY + 31 + (menuActionRow - 1 - curActionIndex) * 15;
			int j2 = 16777215;
			if (mouseX > menuOffsetX && mouseX < menuOffsetX + menuWidth && mouseY > i2 - 13 && mouseY < i2 + 3)
			{
				j2 = 16776960;
			}
			chatTextDrawingArea.method389(true, menuOffsetX + 3, j2, menuActionNames[curActionIndex], i2);
		}
	}
	...
	public void addFriend(long playerID)
	{
		try 
		{
			...
			if (mFriendsCount >= 100 && mMemberStatus != 1)
			{
				pushMessage("Your friendlist is full. Max of 100 for free users, and 200 for members", 0, "");
				return;
			}
			if (mFriendsCount >= 200) 
			{
				pushMessage("Your friendlist is full. Max of 100 for free users, and 200 for members", 0, "");
				return;
			}
			String playerName = TextClass.fixName(TextClass.nameForLong(playerID));
			for (int i = 0; i < mFriendsCount; i++)
			{
				if (mFriendsListAsLongs[i] == playerID)
				{
					pushMessage(playerName + " is already on your friend list", 0, "");
					return;
				}
			}
			for (int j = 0; j < ignoreCount; j++) 
			{
				if (ignoreListAsLongs[j] == playerID) 
				{
					pushMessage("Please remove " + playerName + " from your ignore list first", 0, "");
					return;
				}
			}

			if (playerName.equals(myPlayer.name)) 
			{
				return;
			} 
			else
			{
				mFriendsList[mFriendsCount] = playerName;
				mFriendsListAsLongs[mFriendsCount] = playerID;
				mFriendsNodeIDs[mFriendsCount] = 0;
				mFriendsCount++;
				mNeedDrawTabArea = true;
				mStream.createFrame(188);
				mStream.writeQWord(playerID);
				return;
			}
		} 
		...
		{
			Signlink.reporterror("15283, " + (byte) 68 + ", " + playerID + ", " + runtimeexception.toString());
		}
		throw new RuntimeException();
	}
	...
	public static String intToKOrMil(int num) 
	{
		if (num < 100000) 
		{
	...
	public void resetLogout()
	{
		...
		nextSong = -1;
		previousSong = 0;
		setNextSong(10, musicVolume, false, 0);
	}
	...
	public void method46(int i, Stream stream) 
	{
		while (stream.bitPosition + 21 < i * 8) 
		{
			...
			if (npcArray[npcIndex] == null) 
			{
				npcArray[npcIndex] = new NPC();
			}
			NPC npc = npcArray[npcIndex];
			mNpcIndices[mNumNpcs++] = npcIndex;
			npc.loopCycle = loopCycle;
			int yOffset = stream.readBits(5);
			if (yOffset > 15)
			{
				yOffset -= 32;
			}
			int xOffset = stream.readBits(5);
			if (xOffset > 15)
			{
				xOffset -= 32;
			}
			...
			npc.setPos(myPlayer.smallX[0] + xOffset, myPlayer.smallY[0] + yOffset, j1 == 1);
		}
	...
	public void method47(boolean flag)
	{
		...
		for (int i = 0; i < playerCount; i++) 
		{
			...
			{
				player = myPlayer;
				myPlayerIndex = mMyPlayerIndex << 14;
			} 
			else 
			{
				player = mPlayerArray[mPlayerIndices[i]];
				myPlayerIndex = mPlayerIndices[i] << 14;
			}
			if (player == null || !player.isVisible())
			{
				continue;
			}
			player.aBoolean1699 = (mLowMem && mPlayerCount > 50 || mPlayerCount > 200) && !flag && player.anInt1517 == player.anInt1511;
			int j1 = player.x >> 7;
			int k1 = player.y >> 7;
			...
			if (player.aModel_1714 != null && loopCycle >= player.anInt1707 && loopCycle < player.anInt1708) 
			{
				...
				worldController.method286(plane, player.y, player, player.anInt1552, player.anInt1722, player.x, player.anInt1709, player.anInt1719, player.anInt1721, myPlayerIndex, player.anInt1720);
				continue;
			}
			...
			player.anInt1709 = method42(plane, player.y, player.x);
			worldController.method285(plane, player.anInt1552, player.anInt1709, myPlayerIndex, player.y, 60, player.x, player, player.aBoolean1541);
		}
	}

	public boolean promptUserForInput(RSInterface userInterface)
	{
		int actionID = userInterface.actionID;
		if (anInt900 == 2)
		{
			if (actionID == 201)
			{
				...
				promptInput = "";
				friendsListAction = 1;
				friendAddString = "Enter name of friend to add to list";
			}
			if (actionID == 202)
			{
				inputTaken = true;
				...
				promptInput = "";
				friendsListAction = 2;
				friendAddString = "Enter name of friend to delete from list";
			}
		}
		if (actionID == 205) 
		{
		...
		if (actionID == 501) 
		{
			...
			promptInput = "";
			friendsListAction = 4;
			friendAddString = "Enter name of player to add to list";
		}
		if (actionID == 502) 
		{
			inputTaken = true;
			...
			promptInput = "";
			friendsListAction = 5;
			friendAddString = "Enter name of player to delete from list";
		}
		if (actionID >= 300 && actionID <= 313) 
		{
			int k = (actionID - 300) / 2;
			int j1 = actionID & 1;
		...
		if (actionID >= 314 && actionID <= 323) 
		{
			int l = (actionID - 314) / 2;
			int k1 = actionID & 1;
		...
		if (actionID == 324 && !aBoolean1047)
		{
			aBoolean1047 = true;
			method45();
		}
		if (actionID == 325 && aBoolean1047) 
		{
			aBoolean1047 = false;
			method45();
		}
		if (actionID == 326)
		{
			mStream.createFrame(101);
			mStream.writeWordBigEndian(aBoolean1047 ? 0 : 1);
			for (int i1 = 0; i1 < 7; i1++)
			{
				mStream.writeWordBigEndian(anIntArray1065[i1]);
			}

			for (int l1 = 0; l1 < 5; l1++) 
			{
				mStream.writeWordBigEndian(anIntArray990[l1]);
			}
		...
		if (actionID >= 601 && actionID <= 612) 
		{
			closeOpenInterfaces();
			if (reportAbuseInput.length() > 0)
			{
				mStream.createFrame(218);
				mStream.writeQWord(TextClass.longForName(reportAbuseInput));
				mStream.writeWordBigEndian(actionID - 601);
				mStream.writeWordBigEndian(canMute ? 1 : 0);
			}
		}
		return false;
	}
	...
	public void method49(Stream stream) 
	{
		...
		{
			int playerIndex = anIntArray894[j];
			Player player = mPlayerArray[playerIndex];
			int l = stream.readUnsignedByte();
			if ((l & 0x40) != 0) 
			{
				l += stream.readUnsignedByte() << 8;
			}
			method107(l, playerIndex, stream, player);
		}
	...
	public void method50(int i, int k, int l, int i1, int j1) 
	{
		...
		if (k1 != 0)
		{
			...
			int ai[] = mSomekindOfSprite.pixels;
		...
		if (k1 != 0) 
		{
			...
			else if (j3 == 9)
			{
				...
				int ai1[] = mSomekindOfSprite.pixels;
	...
	{
		mTitleboxBackground = new Background(titleStreamLoader, "titlebox", 0);
		mTitlebuttonBackground = new Background(titleStreamLoader, "titlebutton", 0);
		mRuneBackgrounds = new Background[12];
		int fl_icon = 0;
		try 
		{
			fl_icon = Integer.parseInt(getParameter("fl_icon"));
		} 
		catch (Exception _ex) 
		{
		...
		if (fl_icon == 0) {
			for (int i = 0; i < 12; i++)
			{
				mRuneBackgrounds[i] = new Background(titleStreamLoader, "runes", i);
			}
		...
		else
		{
			for (int i = 0; i < 12; i++)
			{
				mRuneBackgrounds[i] = new Background(titleStreamLoader, "runes", 12 + (i & 3));
			}
		}
		mSprite1 = new Sprite(128, 265);
		mSprite2 = new Sprite(128, 265);
		System.arraycopy(mSprite1ImageProducer.pixels, 0, mSprite1.pixels, 0, 33920);

		System.arraycopy(mSprite2ImageProducer.pixels, 0, mSprite2.pixels, 0, 33920);

		mBuffer1 = new int[256];
		for (int i = 0; i < 64; i++)
		{
			mBuffer1[i] = i * 0x40000;
		}

		for (int i = 0; i < 64; i++) 
		{
			mBuffer1[i + 64] = 0xff0000 + 1024 * i;
		}

		for (int i = 0; i < 64; i++) {
			mBuffer1[i + 128] = 0xffff00 + 4 * i;
		}

		for (int i = 0; i < 64; i++) {
			mBuffer1[i + 192] = 0xffffff;
		}

		mBuffer2 = new int[256];
		for (int i = 0; i < 64; i++) {
			mBuffer2[i] = i * 1024;
		}

		for (int i = 0; i < 64; i++) {
			mBuffer2[i + 64] = 65280 + 4 * i;
		}

		for (int i = 0; i < 64; i++) {
			mBuffer2[i + 128] = 65535 + 0x40000 * i;
		}

		for (int i = 0; i < 64; i++) {
			mBuffer2[i + 192] = 0xffffff;
		}

		mBuffer3 = new int[256];
		for (int i = 0; i < 64; i++) {
			mBuffer3[i] = i * 4;
		}

		for (int i = 0; i < 64; i++) {
			mBuffer3[i + 64] = 255 + 0x40000 * i;
		}

		for (int i = 0; i < 64; i++) {
			mBuffer3[i + 128] = 0xff00ff + 1024 * i;
		}

		for (int i = 0; i < 64; i++) {
			mBuffer3[i + 192] = 0xffffff;
		}

		mBuffer4 = new int[256];
		mBuffer5 = new int[32768];
		mBuffer6 = new int[32768];
		randomizeBackground(null);
		mBuffer7 = new int[32768];
		mBuffer8 = new int[32768];
		drawLoadingText(10, "Connecting to fileserver");
		if (!aBoolean831)
		{
			drawFlames = true;
			aBoolean831 = true;
			startRunnableThread(this, 2);
		}
	}
	...
	{
		if (mLowMem && mLoadingStage == 2 && ObjectManager.plane != plane) 
		{
			drawTextOnScreen(null, "Loading - please wait.");
			mLoadingStage = 1;
			mCurrentTimeMilli = System.currentTimeMillis();
		}
		if (mLoadingStage == 1)
		{
			int j = method54();
			if (j != 0 && System.currentTimeMillis() - mCurrentTimeMilli > 0x57e40L) 
			{
				Signlink.reporterror(myUsername + " glcfb " + aLong1215 + "," + j + "," + mLowMem + "," + decompressors[0] + "," + onDemandFetcher.getNodeCount() + "," + plane + "," + anInt1069 + "," + anInt1070);
				mCurrentTimeMilli = System.currentTimeMillis();
			}
		}
		if (mLoadingStage == 2 && plane != anInt985)
		{
			anInt985 = plane;
			method24(plane);
		}
	...
	{
		for (Animable_Sub4 animableSub4 = (Animable_Sub4) mNodeList2.reverseGetFirst(); animableSub4 != null; animableSub4 = (Animable_Sub4) mNodeList2.reverseGetNext())
		{
			if (animableSub4.anInt1597 != plane || loopCycle > animableSub4.anInt1572) 
			{
				animableSub4.unlink();
			} 
			else if (loopCycle >= animableSub4.anInt1571) 
			{
				if (animableSub4.anInt1590 > 0) 
				{
					NPC npc = npcArray[animableSub4.anInt1590 - 1];
					if (npc != null && npc.x >= 0 && npc.x < 13312 && npc.y >= 0 && npc.y < 13312) 
					{
						animableSub4.method455(loopCycle, npc.y, method42(animableSub4.anInt1597, npc.y, npc.x) - animableSub4.anInt1583, npc.x);
					}
				}
				if (animableSub4.anInt1590 < 0) 
				{
					int j = -animableSub4.anInt1590 - 1;
					Player player;
					if (j == unknownInt10)
					{
						player = myPlayer;
					...
					{
						player = mPlayerArray[j];
					}
					if (player != null && player.x >= 0 && player.x < 13312 && player.y >= 0 && player.y < 13312) 
					{
						animableSub4.method455(loopCycle, player.y, method42(animableSub4.anInt1597, player.y, player.x) - animableSub4.anInt1583, player.x);
					}
				}
				animableSub4.method456(anInt945);
				worldController.method285(plane, animableSub4.anInt1595, (int) animableSub4.aDouble1587, -1, (int) animableSub4.aDouble1586, 60, (int) animableSub4.aDouble1585, animableSub4, false);
			}
		}
	...
	{
		byte titleRawData[] = titleStreamLoader.getDataForName("title.dat");
		Sprite titleSprite = new Sprite(titleRawData, this);
		mSprite1ImageProducer.initDrawingArea();
		titleSprite.setPosition(0, 0);
		mSprite2ImageProducer.initDrawingArea();
		titleSprite.setPosition(-637, 0);
		aRSImageProducer_1107.initDrawingArea();
		titleSprite.setPosition(-128, 0);
		aRSImageProducer_1108.initDrawingArea();
		titleSprite.setPosition(-202, -371);
		aRSImageProducer_1109.initDrawingArea();
		titleSprite.setPosition(-202, -171);
		aRSImageProducer_1112.initDrawingArea();
		titleSprite.setPosition(0, -265);
		aRSImageProducer_1113.initDrawingArea();
		titleSprite.setPosition(-562, -265);
		aRSImageProducer_1114.initDrawingArea();
		titleSprite.setPosition(-128, -171);
		aRSImageProducer_1115.initDrawingArea();
		titleSprite.setPosition(-562, -171);
		
		int pixelbuffer[] = new int[titleSprite.width];
		for (int i = 0; i < titleSprite.height; i++)
		{
			for (int j = 0; j < titleSprite.width; j++) 
			{
				pixelbuffer[j] = titleSprite.pixels[titleSprite.width - j - 1 + titleSprite.width * i];
			}

			System.arraycopy(pixelbuffer, 0, titleSprite.pixels, titleSprite.width * i, titleSprite.width);
		}

		mSprite1ImageProducer.initDrawingArea();
		titleSprite.setPosition(382, 0);
		mSprite2ImageProducer.initDrawingArea();
		titleSprite.setPosition(-255, 0);
		aRSImageProducer_1107.initDrawingArea();
		titleSprite.setPosition(254, 0);
		aRSImageProducer_1108.initDrawingArea();
		titleSprite.setPosition(180, -371);
		aRSImageProducer_1109.initDrawingArea();
		titleSprite.setPosition(180, -171);
		aRSImageProducer_1112.initDrawingArea();
		titleSprite.setPosition(382, -265);
		aRSImageProducer_1113.initDrawingArea();
		titleSprite.setPosition(-180, -265);
		aRSImageProducer_1114.initDrawingArea();
		titleSprite.setPosition(254, -171);
		aRSImageProducer_1115.initDrawingArea();
		titleSprite.setPosition(-180, -171);
		titleSprite = new Sprite(titleStreamLoader, "logo", 0);
		aRSImageProducer_1107.initDrawingArea();
		titleSprite.drawSprite(382 - titleSprite.width / 2 - 128, 18);
		titleSprite = null;
		System.gc();
	...
	public void processOnDemandQueue() 
	{
		do 
		{
			...
			do 
			{
				...
				if (onDemandData.dataType == 3 && mLoadingStage == 1) 
				{
					for (int i = 0; i < aByteArrayArray1183.length; i++) 
					{
	...
	public void calcFlamesPosition()
	{
		...
		for (int j1 = 1; j1 < c - 1; j1++) 
		{
			...
			{
				int l2 = l1 + (j1 << 7);
				mBuffer8[l2] = (mBuffer7[l2 - 1] + mBuffer7[l2 + 1] + mBuffer7[l2 - 128] + mBuffer7[l2 + 128]) / 4;
			}
		}

		anInt1275 += 128;
		if (anInt1275 > mBuffer5.length)
		{
			anInt1275 -= mBuffer5.length;
			int i2 = (int) (Math.random() * 12D);
			randomizeBackground(mRuneBackgrounds[i2]);
		}
		for (int j2 = 1; j2 < c - 1; j2++) 
		{
			...
			{
				int k3 = i3 + (j2 << 7);
				int i4 = mBuffer8[k3 + 128] - mBuffer5[k3 + anInt1275 & mBuffer5.length - 1] / 5;
				if (i4 < 0) 
				{
					i4 = 0;
				}
	...
	{
		RSInterface interfaceMenu = RSInterface.interfaceCache[i];
		for (int childIndex : interfaceMenu.children) 
		{
			if (childIndex == -1) 
			{
				break;
			}
			RSInterface childInterfaceMenu = RSInterface.interfaceCache[childIndex];
			if (childInterfaceMenu.type == 1) 
			{
				method60(childInterfaceMenu.id);
			}
			childInterfaceMenu.anInt246 = 0;
			childInterfaceMenu.anInt208 = 0;
		}
	}
	...
	public void mainGameProcessor()
	{
		...
		synchronized (mouseDetection.syncObject)
		{
			if (flagged)
			{
				...
				{
					mStream.createFrame(45);
					mStream.writeWordBigEndian(0);
					int currentOffset = mStream.currentOffset;
					int j3 = 0;
					for (int j4 = 0; j4 < mouseDetection.coordsIndex; j4++) 
					{
						if (currentOffset - mStream.currentOffset >= 240)
						{
							break;
						}
						j3++;
						int coordY = mouseDetection.coordsY[j4];
						if (coordY < 0) 
						{
							coordY = 0;
						} else if (coordY > 502)
						...
						int coordX = mouseDetection.coordsX[j4];
						if (coordX < 0) 
						{
							coordX = 0;
						} 
						...
						if (coordX == anInt1237 && coordY == anInt1238) 
						{
							if (anInt1022 < 2047) 
							{
								anInt1022++;
						...
						{
							int j6 = coordX - anInt1237;
							anInt1237 = coordX;
							int k6 = coordY - anInt1238;
							anInt1238 = coordY;
							if (anInt1022 < 8 && j6 >= -32 && j6 <= 31 && k6 >= -32 && k6 <= 31) 
							{
								j6 += 32;
								k6 += 32;
								mStream.writeWord((anInt1022 << 12) + (j6 << 6) + k6);
								anInt1022 = 0;
							} 
							else if (anInt1022 < 8) 
							{
								mStream.writeDWordBigEndian(0x800000 + (anInt1022 << 19) + i6);
								anInt1022 = 0;
							} 
							else 
							{
								mStream.writeDWord(0xc0000000 + (anInt1022 << 19) + i6);
								anInt1022 = 0;
							}
						}
					}

					mStream.writeBytes(mStream.currentOffset - currentOffset);
		...
		if (super.clickMode3 != 0) 
		{
			...
			int k4 = saveClickY * 765 + saveClickX;
			int j5 = 0;
			...
			int l5 = (int) l;
			mStream.createFrame(241);
			mStream.writeDWord((l5 << 20) + (j5 << 19) + k4);
		}
		if (anInt1016 > 0)
		{
			anInt1016--;
		...
		if (aBoolean1017 && anInt1016 <= 0) 
		{
			anInt1016 = 20;
			aBoolean1017 = false;
			mStream.createFrame(86);
			mStream.writeWord(anInt1184);
			mStream.method432(minimapInt1);
		}
		if (super.awtFocus && !aBoolean954) 
		{
			aBoolean954 = true;
			mStream.createFrame(3);
			mStream.writeWordBigEndian(1);
		}
		if (!super.awtFocus && aBoolean954) 
		{
			aBoolean954 = false;
		...
		{
			anInt989++;
			if (super.mouseX > lastSaveClickX + 5 || super.mouseX < lastSaveClickX - 5 || super.mouseY > lastSaveClickY + 5 || super.mouseY < lastSaveClickY - 5)
			{
				aBoolean1242 = true;
			}
			if (super.clickMode2 == 0)
			{
				...
				if (aBoolean1242 && anInt989 >= 5)
				{
					...
					processRightClick();
					if (lastActiveInvInterface == lastMenuAction3 && mMouseInvInterfaceIndex != mLastMenuAction2)
					{
						RSInterface menuInterface = RSInterface.interfaceCache[lastMenuAction3];
						int j1 = 0;
						if (anInt913 == 1 && menuInterface.actionID == 206) 
						{
							j1 = 1;
						}
						if (menuInterface.inv[mMouseInvInterfaceIndex] <= 0)
						{
							j1 = 0;
						}
						if (menuInterface.aBoolean235)
						{
							int lastMenuAction2 = mLastMenuAction2;
							int mouseInvInterfaceIndex = mMouseInvInterfaceIndex;
							menuInterface.inv[mouseInvInterfaceIndex] = menuInterface.inv[lastMenuAction2];
							menuInterface.invStackSizes[mouseInvInterfaceIndex] = menuInterface.invStackSizes[lastMenuAction2];
							menuInterface.inv[lastMenuAction2] = -1;
							menuInterface.invStackSizes[lastMenuAction2] = 0;
						} 
						else if (j1 == 1) 
						{
							int lastMenuAction2 = mLastMenuAction2;
							for (int i4 = mMouseInvInterfaceIndex; lastMenuAction2 != i4;) 
							{
								if (lastMenuAction2 > i4) 
								{
									menuInterface.swapInventoryItems(lastMenuAction2, lastMenuAction2 - 1);
									lastMenuAction2--;
								} 
								else if (lastMenuAction2 < i4)
								{
									menuInterface.swapInventoryItems(lastMenuAction2, lastMenuAction2 + 1);
									lastMenuAction2++;
								}
						...
						{
							menuInterface.swapInventoryItems(mLastMenuAction2, mMouseInvInterfaceIndex);
						}
						mStream.createFrame(214);
						mStream.method433(lastMenuAction3);
						mStream.method424(j1);
						mStream.method433(mLastMenuAction2);
						mStream.method431(mMouseInvInterfaceIndex);
					}
				}
		...
		if (mLoadingStage == 2 && aBoolean1160) 
		{
			calcCameraPos();
		}
		...
		try 
		{
			if (socketStream != null && mStream.currentOffset > 0)
			{
				socketStream.queueBytes(mStream.currentOffset, mStream.buffer);
				mStream.currentOffset = 0;
				anInt1010 = 0;
			}
		} 
	...
	void drawLoadingText(int i, String s) {
		...
		if (welcomeScreenRaised) {
			...
			if (!aBoolean831) {
				mSprite1ImageProducer.drawGraphics(0, super.graphics, 0);
				mSprite2ImageProducer.drawGraphics(0, super.graphics, 637);
			}
	...
	public StreamLoader streamLoaderForName(int i, String s, String s1, int j, int k) {
		...
		while (abyte0 == null) {
			...
			try {
				...
				try {
					if (decompressors[0] != null) {
						decompressors[0].decompressSound(abyte0.length, abyte0, i);
					}
				} catch (Exception _ex) {
	...
	public void drawTextOnScreen(String s, String s1) {
		if (aRSImageProducer_1165 != null) {
			...
			if (s != null)
				j -= 7;
			mChatDrawingArea.drawText(0, s1, j, 257);
			mChatDrawingArea.drawText(0xffffff, s1, j - 1, 256);
			j += 15;
			if (s != null) {
				mChatDrawingArea.drawText(0, s, j, 257);
				mChatDrawingArea.drawText(0xffffff, s, j - 1, 256);
			}
		...
		if (super.fullGameScreen != null) {
			...
			char c = '\u012C';
			byte byte0 = 50;
			DrawingArea.method336(byte0, k - 5 - byte0 / 2, 0, c, 383 - c / 2);
			DrawingArea.fillPixels(k - 5 - byte0 / 2, byte0, 0xffffff, 383 - c / 2, c);
			if (s != null)
				k -= 7;
			mChatDrawingArea.drawText(0, s1, k, 383);
			mChatDrawingArea.drawText(0xffffff, s1, k - 1, 382);
			k += 15;
			if (s != null) {
				mChatDrawingArea.drawText(0, s, k, 383);
				mChatDrawingArea.drawText(0xffffff, s, k - 1, 382);
			}
	...
	public void doAction(int i) {
		...
		int j = menuInventorySlots[i];
		int k = menuInvInterfaceIDs[i];
		int l = menuActionIDs[i];
		int i1 = menuItemDefIDs[i];
		if (l >= 2000) {
			l -= 2000;
		}
		if (l == 582) {
			...
			if (npc != null) {
				...
				crossType = 2;
				crossIndex = 0;
				mStream.createFrame(57);
				mStream.method432(anInt1285);
				mStream.method432(i1);
				mStream.method431(anInt1283);
				mStream.method432(anInt1284);
			}
		}
		if (l == 234) {
			...
			crossType = 2;
			crossIndex = 0;
			mStream.createFrame(236);
			mStream.method431(k + baseY);
			mStream.writeWord(i1);
			mStream.method431(j + baseX);
		}
		if (l == 62 && method66(i1, k, j)) {
			mStream.createFrame(192);
			mStream.writeWord(anInt1284);
			mStream.method431(i1 >> 14 & 0x7fff);
			mStream.method433(k + baseY);
			mStream.method431(anInt1283);
			mStream.method433(j + baseX);
			mStream.writeWord(anInt1285);
		}
		if (l == 511) {
			...
			crossType = 2;
			crossIndex = 0;
			mStream.createFrame(25);
			mStream.method431(anInt1284);
			mStream.method432(anInt1285);
			mStream.writeWord(i1);
			mStream.method432(k + baseY);
			mStream.method433(anInt1283);
			mStream.writeWord(j + baseX);
		}
		if (l == 74) {
			mStream.createFrame(122);
			mStream.method433(k);
			mStream.method432(j);
			mStream.method431(i1);
		...
		if (l == 315) {
			...
			boolean flag8 = true;
			if (class9.actionID > 0) {
				flag8 = promptUserForInput(class9);
			}
			if (flag8) {
				mStream.createFrame(185);
				mStream.writeWord(k);
			}
		}
		if (l == 561) {
			Player player = mPlayerArray[i1];
			if (player != null) {
				...
				anInt1188 += i1;
				if (anInt1188 >= 90) {
					mStream.createFrame(136);
					anInt1188 = 0;
				}
				mStream.createFrame(128);
				mStream.writeWord(i1);
			}
		}
		if (l == 20) {
			...
			if (class30_sub2_sub4_sub1_sub1_1 != null) {
				...
				crossType = 2;
				crossIndex = 0;
				mStream.createFrame(155);
				mStream.method431(i1);
			}
		}
		if (l == 779) {
			Player class30_sub2_sub4_sub1_sub2_1 = mPlayerArray[i1];
		...
		if (l == 1062) {
			anInt924 += baseX;
			if (anInt924 >= 113) {
				mStream.createFrame(183);
				mStream.writeDWordBigEndian(0xe63271);
				anInt924 = 0;
			}
			method66(i1, k, j);
			mStream.createFrame(228);
			mStream.method432(i1 >> 14 & 0x7fff);
			mStream.method432(k + baseY);
			mStream.writeWord(j + baseX);
		}
		if (l == 679 && !aBoolean1149) {
			mStream.createFrame(40);
			mStream.writeWord(k);
			aBoolean1149 = true;
		}
		if (l == 431) {
			mStream.createFrame(129);
			mStream.method432(j);
			mStream.writeWord(k);
			mStream.method432(i1);
			atInventoryLoopCycle = 0;
		...
		if (l == 484 || l == 6) {
			String s1 = menuActionNames[i];
			int l1 = s1.indexOf("@whi@");
			if (l1 != -1) {
				s1 = s1.substring(l1 + 5).trim();
				String s7 = TextClass.fixName(TextClass.nameForLong(TextClass.longForName(s1)));
				boolean flag9 = false;
				for (int j3 = 0; j3 < mPlayerCount; j3++) {
					Player class30_sub2_sub4_sub1_sub2_7 = mPlayerArray[mPlayerIndices[j3]];
					...
					if (l == 484) {
						mStream.createFrame(139);
						mStream.method431(mPlayerIndices[j3]);
					}
					if (l == 6) {
						anInt1188 += i1;
						if (anInt1188 >= 90) {
						...
						mStream.createFrame(128);
						mStream.writeWord(mPlayerIndices[j3]);
					}
					flag9 = true;
					break;
				}
		...
		if (l == 870) {
			mStream.createFrame(53);
			mStream.writeWord(j);
			mStream.method432(anInt1283);
			mStream.method433(i1);
			mStream.writeWord(anInt1284);
			mStream.method431(anInt1285);
			mStream.writeWord(k);
			atInventoryLoopCycle = 0;
			atInventoryInterface = k;
			atInventoryIndex = j;
		...
		if (l == 27) {
			Player class30_sub2_sub4_sub1_sub2_2 = mPlayerArray[i1];
			if (class30_sub2_sub4_sub1_sub2_2 != null) {
				...
				anInt986 += i1;
				if (anInt986 >= 54) {
					mStream.createFrame(189);
					mStream.writeWordBigEndian(234);
					anInt986 = 0;
				}
				mStream.createFrame(73);
				mStream.method431(i1);
			}
		}
		if (l == 213) {
			...
			crossType = 2;
			crossIndex = 0;
			mStream.createFrame(79);
			mStream.method431(k + baseY);
			mStream.writeWord(i1);
			mStream.method432(j + baseX);
		}
		if (l == 632) {
			mStream.createFrame(145);
			mStream.method432(k);
			mStream.method432(j);
			mStream.method432(i1);
			atInventoryLoopCycle = 0;
		...
		if (l == 94) {
			...
			crossType = 2;
			crossIndex = 0;
			mStream.createFrame(181);
			mStream.method431(k + baseY);
			mStream.writeWord(i1);
			mStream.method431(j + baseX);
			mStream.method432(anInt1137);
		}
		if (l == 646) {
			mStream.createFrame(185);
			mStream.writeWord(k);
		...
		if (l == 225) {
			...
			if (class30_sub2_sub4_sub1_sub1_2 != null) {
				...
				anInt1226 += i1;
				if (anInt1226 >= 85) {
					mStream.createFrame(230);
					mStream.writeWordBigEndian(239);
					anInt1226 = 0;
				}
				mStream.createFrame(17);
				mStream.method433(i1);
			}
		}
		if (l == 965) {
			...
			if (class30_sub2_sub4_sub1_sub1_3 != null) {
				...
				anInt1134++;
				if (anInt1134 >= 96) {
					mStream.createFrame(152);
					mStream.writeWordBigEndian(88);
					anInt1134 = 0;
				}
				mStream.createFrame(21);
				mStream.writeWord(i1);
			}
		}
		if (l == 413) {
		...
		if (l == 900) {
			method66(i1, k, j);
			mStream.createFrame(252);
			mStream.method433(i1 >> 14 & 0x7fff);
			mStream.method431(k + baseY);
			mStream.method432(j + baseX);
		}
		if (l == 412) {
			...
			if (class30_sub2_sub4_sub1_sub1_6 != null) {
				...
				crossType = 2;
				crossIndex = 0;
				mStream.createFrame(72);
				mStream.method432(i1);
			}
		}
		if (l == 365) {
			Player class30_sub2_sub4_sub1_sub2_3 = mPlayerArray[i1];
			if (class30_sub2_sub4_sub1_sub2_3 != null) {
				...
				crossType = 2;
				crossIndex = 0;
				mStream.createFrame(249);
				mStream.method432(i1);
				mStream.method431(anInt1137);
			}
		}
		if (l == 729) {
			Player class30_sub2_sub4_sub1_sub2_4 = mPlayerArray[i1];
			if (class30_sub2_sub4_sub1_sub2_4 != null) {
				...
				crossType = 2;
				crossIndex = 0;
				mStream.createFrame(39);
				mStream.method431(i1);
			}
		}
		if (l == 577) {
			Player class30_sub2_sub4_sub1_sub2_5 = mPlayerArray[i1];
		...
		if (l == 867) {
			...
			if (anInt1175 >= 59) {
				mStream.createFrame(200);
				mStream.writeWord(25501);
				anInt1175 = 0;
			}
			mStream.createFrame(43);
			mStream.method431(k);
			mStream.method432(i1);
			mStream.method432(j);
			atInventoryLoopCycle = 0;
		...
		if (l == 491) {
			Player class30_sub2_sub4_sub1_sub2_6 = mPlayerArray[i1];
		...
		if (l == 639) {
			String s3 = menuActionNames[i];
			int k2 = s3.indexOf("@whi@");
			if (k2 != -1) {
				long l4 = TextClass.longForName(s3.substring(k2 + 5).trim());
				int k3 = -1;
				for (int i4 = 0; i4 < mFriendsCount; i4++) {
					if (mFriendsListAsLongs[i4] != l4) {
						continue;
					}
					k3 = i4;
					break;
				...
				if (k3 != -1 && mFriendsNodeIDs[k3] > 9) {
					inputTaken = true;
					inputDialogState = 0;
					messagePromptRaised = true;
					promptInput = "";
					friendsListAction = 3;
					aLong953 = mFriendsListAsLongs[k3];
					friendAddString = "Enter message to send to " + mFriendsList[k3];
				} else {
		...
		if (l == 478) {
			...
			if (class30_sub2_sub4_sub1_sub1_7 != null) {
				...
				if (anInt1155 >= 53) {
					mStream.createFrame(85);
					mStream.writeWordBigEndian(66);
					anInt1155 = 0;
				}
				mStream.createFrame(18);
				mStream.method431(i1);
			}
		}
		if (l == 113) {//third
			method66(i1, k, j);
			mStream.createFrame(70);
			mStream.method431(j + baseX);
			mStream.writeWord(k + baseY);
			mStream.method433(i1 >> 14 & 0x7fff);
		}
		if (l == 872) {//fourth
			method66(i1, k, j);
			mStream.createFrame(234);
			mStream.method433(j + baseX);
			mStream.method432(i1 >> 14 & 0x7fff);
			mStream.method433(k + baseY);
		}
		if (l == 502) {//first
			method66(i1, k, j);
			mStream.createFrame(132);
			mStream.method433(j + baseX);
			mStream.writeWord(i1 >> 14 & 0x7fff);
			mStream.method432(k + baseY);
		}
		if (l == 1125) {
	...
	public void build3dScreenMenu() {
		if (itemSelected == 0 && spellSelected == 0) {
			...
			menuInventorySlots[menuActionRow] = super.mouseX;
			menuInvInterfaceIDs[menuActionRow] = super.mouseY;
			menuActionRow++;
		}
		int j = -1;
		for (int k = 0; k < Model.anInt1687; k++) {
			...
			if (k1 == 2 && worldController.method304(plane, i1, j1, l) >= 0) {
				...
				if (itemSelected == 1) {
					menuActionNames[menuActionRow] = "Use " + selectedItemName + " with @cya@" + class46.name;
					menuActionIDs[menuActionRow] = 62;
					menuItemDefIDs[menuActionRow] = l;
					menuInventorySlots[menuActionRow] = i1;
					menuInvInterfaceIDs[menuActionRow] = j1;
					menuActionRow++;
				} else if (spellSelected == 1) {
					if ((spellUsableOn & 4) == 4) {
						menuActionNames[menuActionRow] = spellTooltip + " @cya@" + class46.name;
					...
					if (class46.actions != null) {
						for (int i2 = 4; i2 >= 0; i2--) {
							if (class46.actions[i2] != null) {
								menuActionNames[menuActionRow] = class46.actions[i2] + " @cya@" + class46.name;
								if (i2 == 0) {
					...
					menuActionNames[menuActionRow] = "Examine @cya@" + class46.name + (showInfo ? " @gre@(@whi@" + l1 + "@gre@) (@whi@" + (i1 + baseX) + "," + (j1 + baseY) + "@gre@)" : "");
					menuActionIDs[menuActionRow] = 1226;
					menuItemDefIDs[menuActionRow] = class46.type << 14;
					menuInventorySlots[menuActionRow] = i1;
					menuInvInterfaceIDs[menuActionRow] = j1;
					menuActionRow++;
				}
			...
			if (k1 == 1) {
				...
				if (npc.desc.aByte68 == 1 && (npc.x & 0x7f) == 64 && (npc.y & 0x7f) == 64) {
					for (int j2 = 0; j2 < mNumNpcs; j2++) {
						NPC npc2 = npcArray[mNpcIndices[j2]];
						if (npc2 != null && npc2 != npc && npc2.desc.aByte68 == 1 && npc2.x == npc.x && npc2.y == npc.y) {
							buildAtNPCMenu(npc2.desc, mNpcIndices[j2], j1, i1);
						}
					}

					for (int l2 = 0; l2 < mPlayerCount; l2++) {
						Player player = mPlayerArray[mPlayerIndices[l2]];
						if (player != null && player.x == npc.x && player.y == npc.y) {
							buildAtPlayerMenu(i1, mPlayerIndices[l2], player, j1);
						}
					}
			...
			if (k1 == 0) {
				Player player = mPlayerArray[l1];
				if ((player.x & 0x7f) == 64 && (player.y & 0x7f) == 64) {
					for (int k2 = 0; k2 < mNumNpcs; k2++) {
						NPC class30_sub2_sub4_sub1_sub1_2 = npcArray[mNpcIndices[k2]];
						if (class30_sub2_sub4_sub1_sub1_2 != null && class30_sub2_sub4_sub1_sub1_2.desc.aByte68 == 1 && class30_sub2_sub4_sub1_sub1_2.x == player.x && class30_sub2_sub4_sub1_sub1_2.y == player.y) {
							buildAtNPCMenu(class30_sub2_sub4_sub1_sub1_2.desc, mNpcIndices[k2], j1, i1);
						}
					}

					for (int i3 = 0; i3 < mPlayerCount; i3++) {
						Player class30_sub2_sub4_sub1_sub2_2 = mPlayerArray[mPlayerIndices[i3]];
						if (class30_sub2_sub4_sub1_sub2_2 != null && class30_sub2_sub4_sub1_sub2_2 != player && class30_sub2_sub4_sub1_sub2_2.x == player.x && class30_sub2_sub4_sub1_sub2_2.y == player.y) {
							buildAtPlayerMenu(i1, mPlayerIndices[i3], class30_sub2_sub4_sub1_sub2_2, j1);
						}
					}
				...
				if (class19 != null) {
					for (Item item = (Item) class19.getFirst(); item != null; item = (Item) class19.getNext()) {
						...
						if (itemSelected == 1) {
							...
							menuItemDefIDs[menuActionRow] = item.ID;
							menuInventorySlots[menuActionRow] = i1;
							menuInvInterfaceIDs[menuActionRow] = j1;
							menuActionRow++;
						} else if (spellSelected == 1) {
							...
							for (int j3 = 4; j3 >= 0; j3--) {
								if (itemDef.groundActions != null && itemDef.groundActions[j3] != null) {
									menuActionNames[menuActionRow] = itemDef.groundActions[j3] + " @lre@" + itemDef.name;
									if (j3 == 0) {
									...
									menuActionNames[menuActionRow] = "Take @lre@" + itemDef.name;
									menuActionIDs[menuActionRow] = 234;
									menuItemDefIDs[menuActionRow] = item.ID;
									menuInventorySlots[menuActionRow] = i1;
									menuInvInterfaceIDs[menuActionRow] = j1;
							...
							menuActionNames[menuActionRow] = "Examine @lre@" + itemDef.name + (showInfo ? " @gre@(@whi@" + item.ID + "@gre@)" : "");
	...
	public void cleanUpForQuit() {
		...
		mNodeList2 = null;
		mNodeList1 = null;
		menuInventorySlots = null;
		menuInvInterfaceIDs = null;
		menuActionIDs = null;
		menuItemDefIDs = null;
		...
		mSomekindOfSprite = null;
		mFriendsList = null;
		mFriendsListAsLongs = null;
		mFriendsNodeIDs = null;
		mSprite1ImageProducer = null;
		mSprite2ImageProducer = null;
		...
		do {
			...
			if (openInterfaceID != -1 && openInterfaceID == reportAbuseInterfaceID) {
				...
				if (j == 13 || j == 10) {
					...
					if (friendsListAction == 2 && mFriendsCount > 0) {
						long l1 = TextClass.longForName(promptInput);
						delFriend(l1);
					}
					if (friendsListAction == 3 && promptInput.length() > 0) {
						mStream.createFrame(126);
						mStream.writeWordBigEndian(0);
						int k = mStream.currentOffset;
						mStream.writeQWord(aLong953);
						TextInput.method526(promptInput, mStream);
						mStream.writeBytes(mStream.currentOffset - k);
				...
				if (j == 13 || j == 10) {
					if (amountOrNameInput.length() > 0) {
						...
						mStream.createFrame(208);
						mStream.writeDWord(amount);
					}
					inputDialogState = 0;
					inputTaken = true;
				}
				...
				if (j == 13 || j == 10) {
					if (amountOrNameInput.length() > 0) {
						mStream.createFrame(60);
						mStream.writeQWord(TextClass.longForName(amountOrNameInput));
					}
					inputDialogState = 0;
					inputTaken = true;
				}
				...
				if ((j == 13 || j == 10) && inputString.length() > 0) {
							...
							for (int k1 = 0; k1 < 4; k1++) {
								for (int i2 = 1; i2 < 103; i2++) {
									for (int k2 = 1; k2 < 103; k2++) {
										mCollisionMap[k1].mCollisionMapData[i2][k2] = 0;
									}
					...
					if (inputString.startsWith("::")) {
						mStream.createFrame(103);
						mStream.writeWordBigEndian(inputString.length() - 1);
						mStream.writeString(inputString.substring(2));
					} else {
						...
						mStream.createFrame(4);
						mStream.writeWordBigEndian(0);
						int j3 = mStream.currentOffset;
						mStream.method425(i3);
						mStream.method425(j2);
						aStream_834.currentOffset = 0;
						TextInput.method526(inputString, aStream_834);
						mStream.method441(0, aStream_834.buffer, aStream_834.currentOffset);
						mStream.writeBytes(mStream.currentOffset - j3);
	...
	public void buildChatAreaMenu(int j) {
		...
		for (int i1 = 0; i1 < 100; i1++) {
			...
			if ((j1 == 1 || j1 == 2) && (j1 == 1 || publicChatMode == 0 || publicChatMode == 1 && isFriendOrSelf(s))) {
				if (j > k1 - 14 && j <= k1 && !s.equals(myPlayer.name)) {
					if (myPrivilege >= 1 && myPrivilege <= 3) {
						menuActionNames[menuActionRow] = "Report abuse @whi@" + s;
						menuActionIDs[menuActionRow] = 606;
						menuActionRow++;
					}
					menuActionNames[menuActionRow] = "Add ignore @whi@" + s;
					menuActionIDs[menuActionRow] = 42;
					menuActionRow++;
					menuActionNames[menuActionRow] = "Reply to @whi@" + s;
					menuActionIDs[menuActionRow] = 639;
					menuActionRow++;
					menuActionNames[menuActionRow] = "Add friend @whi@" + s;
			...
			if (j1 == 4 && (tradeMode == 0 || tradeMode == 1 && isFriendOrSelf(s))) {
				if (j > k1 - 14 && j <= k1) {
					menuActionNames[menuActionRow] = "Accept trade @whi@" + s;
			...
			if (j1 == 8 && (tradeMode == 0 || tradeMode == 1 && isFriendOrSelf(s))) {
				if (j > k1 - 14 && j <= k1) {
					menuActionNames[menuActionRow] = "Accept challenge @whi@" + s;
	...
	public void drawFriendsListOrWelcomeScreen(RSInterface class9) {
		...
		if (j >= 1 && j <= 100 || j >= 701 && j <= 800) {
			...
			if (j >= k) {
				...
				class9.disabledText = mFriendsList[j];
		...
		if (j >= 101 && j <= 200 || j >= 801 && j <= 900) {
			...
			if (mFriendsNodeIDs[j] == 0) {
				class9.disabledText = "@red@Offline";
			} else if (mFriendsNodeIDs[j] == nodeID) {
	...
	public void drawSplitpublicChat() {
		...
		TextDrawingArea textDrawingArea = mChatDrawingArea;
		int i = 0;
		if (anInt1104 != 0) {
			i = 1;
		}
		for (int j = 0; j < 100; j++) {
			if (chatMessages[j] != null) {
				...
				if ((k == 3 || k == 7) && (k == 7 || privateChatMode == 0 || privateChatMode == 1 && isFriendOrSelf(s))) {
					int l = 329 - i * 13;
					int k1 = 4;
					textDrawingArea.drawTextMessage(0, "From", l, k1);
					textDrawingArea.drawTextMessage(65535, "From", l - 1, k1);
					...
					textDrawingArea.drawTextMessage(0, s + ": " + chatMessages[j], l, k1);
					textDrawingArea.drawTextMessage(65535, s + ": " + chatMessages[j], l - 1, k1);
					if (++i >= 5) {
						return;
					}
				}
				if (k == 5 && privateChatMode < 2) {
					int i1 = 329 - i * 13;
					textDrawingArea.drawTextMessage(0, chatMessages[j], i1, 4);
					textDrawingArea.drawTextMessage(65535, chatMessages[j], i1 - 1, 4);
					if (++i >= 5) {
						return;
					}
				}
				if (k == 6 && privateChatMode < 2) {
					int j1 = 329 - i * 13;
					textDrawingArea.drawTextMessage(0, "To " + s + ": " + chatMessages[j], j1, 4);
					textDrawingArea.drawTextMessage(65535, "To " + s + ": " + chatMessages[j], j1 - 1, 4);
					if (++i >= 5) {
						return;
					}
				}
	...
	public void processTabClick() {
		if (super.clickMode3 == 1) {
			...
			if (anInt1054 == tabID) {
				mStream.createFrame(152);
				mStream.writeWordBigEndian(tabID);
			}
		}
	}
	...
	public void resetImageProducers() {
		...
		aRSImageProducer_1124 = null;
		aRSImageProducer_1125 = null;
		mSprite1ImageProducer = new RSImageProducer(128, 265, getGameComponent());
		DrawingArea.setAllPixelsToZero();
		mSprite2ImageProducer = new RSImageProducer(128, 265, getGameComponent());
		DrawingArea.setAllPixelsToZero();
	...
	public void resetImageProducers2() {
		...
		aRSImageProducer_1114 = null;
		aRSImageProducer_1115 = null;
		chatImageProducer = new RSImageProducer(479, 96, getGameComponent());
		aRSImageProducer_1164 = new RSImageProducer(172, 156, getGameComponent());
		DrawingArea.setAllPixelsToZero();
		mapBack.method361(0, 0);
		tabAreaImageProducer = new RSImageProducer(190, 261, getGameComponent());
		...
		while (!flag) {
			...
			for (int j = 0; j < menuActionRow - 1; j++) {
				if (menuActionIDs[j] < 1000 && menuActionIDs[j + 1] > 1000) {
					String s = menuActionNames[j];
					menuActionNames[j] = menuActionNames[j + 1];
					menuActionNames[j + 1] = s;
					int k = menuActionIDs[j];
					menuActionIDs[j] = menuActionIDs[j + 1];
					menuActionIDs[j + 1] = k;
					k = menuInventorySlots[j];
					menuInventorySlots[j] = menuInventorySlots[j + 1];
					menuInventorySlots[j + 1] = k;
					k = menuInvInterfaceIDs[j];
					menuInvInterfaceIDs[j] = menuInvInterfaceIDs[j + 1];
					menuInvInterfaceIDs[j + 1] = k;
					k = menuItemDefIDs[j];
					menuItemDefIDs[j] = menuItemDefIDs[j + 1];
					menuItemDefIDs[j + 1] = k;
					flag = false;
				}
			}
	...
	public void login(String s, String s1, boolean flag) {
		...
		try {
			...
			int i = (int) (l >> 16 & 31L);
			mStream.currentOffset = 0;
			mStream.writeWordBigEndian(14);
			mStream.writeWordBigEndian(i);
			socketStream.queueBytes(2, mStream.buffer);
			for (int j = 0; j < 8; j++) {
				socketStream.read();
			}
			...
			if (k == 0) {
				...
				ai[3] = (int) aLong1215;
				mStream.currentOffset = 0;
				mStream.writeWordBigEndian(10);
				mStream.writeDWord(ai[0]);
				mStream.writeDWord(ai[1]);
				mStream.writeDWord(ai[2]);
				mStream.writeDWord(ai[3]);
				mStream.writeDWord(ClientSettings.UID);
				mStream.writeString(s);
				mStream.writeString(s1);
				mStream.doKeys();
				...
				aStream_847.writeWordBigEndian(mStream.currentOffset + 36 + 1 + 1 + 2);
				aStream_847.writeWordBigEndian(255);
				aStream_847.writeWord(1);
				aStream_847.writeWordBigEndian(mLowMem ? 1 : 0);
				for (int l1 = 0; l1 < 9; l1++) {
					aStream_847.writeDWord(expectedCRCs[l1]);
				}

				aStream_847.writeBytes(mStream.buffer, mStream.currentOffset, 0);
				mStream.encryption = new ISAACRandomGen(ai);
			...
			if (k == 2) {
				...
				myPlayer = mPlayerArray[mMyPlayerIndex] = new Player();
				mNodeList2.removeAll();
				mNodeList1.removeAll();
	...
	public boolean doWalkTo(int i, int j, int k, int i1, int j1, int k1, int l1, int i2, int j2, boolean flag, int k2) {
		...
		boolean flag1 = false;
		int j4 = bigX.length;
		int ai[][] = mCollisionMap[plane].mCollisionMapData;
		while (i4 != l3) {
			j3 = bigX[i4];
			k3 = bigY[i4];
			i4 = (i4 + 1) % j4;
			...
			if (i1 != 0) {
				if ((i1 < 5 || i1 == 10) && mCollisionMap[plane].method219(k2, j3, k3, j, i1 - 1, i2)) {
					flag1 = true;
					break;
				}
				if (i1 < 10 && mCollisionMap[plane].method220(k2, i2, k3, i1 - 1, j, j3)) {
					flag1 = true;
					break;
				}
			}
			if (k1 != 0 && k != 0 && mCollisionMap[plane].method221(i2, k2, j3, k, l1, k1, k3)) {
				flag1 = true;
				break;
			}
		...
		if (i4 > 0) {
			...
			anInt1288 += k4;
			if (anInt1288 >= 92) {
				mStream.createFrame(36);
				mStream.writeDWord(0);
				anInt1288 = 0;
			}
			if (i == 0) {
				mStream.createFrame(164);
				mStream.writeWordBigEndian(k4 + k4 + 3);
			}
			if (i == 1) {
				mStream.createFrame(248);
				mStream.writeWordBigEndian(k4 + k4 + 3 + 14);
			}
			if (i == 2) {
				mStream.createFrame(98);
				mStream.writeWordBigEndian(k4 + k4 + 3);
			}
			mStream.method433(k6 + baseX);
			destX = bigX[0];
			destY = bigY[0];
			for (int j7 = 1; j7 < k4; j7++) {
				i4--;
				mStream.writeWordBigEndian(bigX[i4] - k6);
				mStream.writeWordBigEndian(bigY[i4] - i7);
			}

			mStream.method431(i7 + baseY);
			mStream.method424(super.keyArray[5] != 1 ? 0 : 1);
			return true;
		}
		return i != 1;
	}
	...
	public void buildAtNPCMenu(EntityDef entityDef, int i, int j, int k) {
		...
		if (itemSelected == 1) {
			menuActionNames[menuActionRow] = "Use " + selectedItemName + " with @yel@" + s;
			menuActionIDs[menuActionRow] = 582;
			menuItemDefIDs[menuActionRow] = i;
			menuInventorySlots[menuActionRow] = k;
			menuInvInterfaceIDs[menuActionRow] = j;
			menuActionRow++;
			return;
		}
		if (spellSelected == 1) {
			if ((spellUsableOn & 2) == 2) {
				menuActionNames[menuActionRow] = spellTooltip + " @yel@" + s;
			...
			if (entityDef.actions != null) {
				for (int l = 4; l >= 0; l--) {
					if (entityDef.actions[l] != null && !entityDef.actions[l].equalsIgnoreCase("attack")) {
						menuActionNames[menuActionRow] = entityDef.actions[l] + " @yel@" + s;
						if (l == 0) {
			...
			if (entityDef.actions != null) {
				for (int i1 = 4; i1 >= 0; i1--) {
					if (entityDef.actions[i1] != null && entityDef.actions[i1].equalsIgnoreCase("attack")) {
						...
						menuActionNames[menuActionRow] = entityDef.actions[i1] + " @yel@" + s;
						if (i1 == 0) {
							menuActionIDs[menuActionRow] = 20 + c;
						}
						if (i1 == 1) {
							menuActionIDs[menuActionRow] = 412 + c;
						}
						if (i1 == 2) {
							menuActionIDs[menuActionRow] = 225 + c;
						}
						if (i1 == 3) {
							menuActionIDs[menuActionRow] = 965 + c;
						}
						if (i1 == 4) {
							menuActionIDs[menuActionRow] = 478 + c;
						}
						menuItemDefIDs[menuActionRow] = i;
						menuInventorySlots[menuActionRow] = k;
						menuInvInterfaceIDs[menuActionRow] = j;
			...
			menuActionNames[menuActionRow] = "Examine @yel@" + s + (showInfo ? " @gre@(@whi@" + entityDef.type + "@gre@)" : "");
	...
	public void buildAtPlayerMenu(int i, int j, Player player, int k) {
		...
		if (itemSelected == 1) {
			menuActionNames[menuActionRow] = "Use " + selectedItemName + " with @whi@" + s;
			menuActionIDs[menuActionRow] = 491;
			menuItemDefIDs[menuActionRow] = j;
			menuInventorySlots[menuActionRow] = i;
			menuInvInterfaceIDs[menuActionRow] = k;
			menuActionRow++;
		} else if (spellSelected == 1) {
			if ((spellUsableOn & 8) == 8) {
				menuActionNames[menuActionRow] = spellTooltip + " @whi@" + s;
			...
			for (int l = 4; l >= 0; l--) {
				if (atPlayerActions[l] != null) {
					menuActionNames[menuActionRow] = atPlayerActions[l] + " @whi@" + s;
					char c = '\0';
					...
					if (l == 0) {
						menuActionIDs[menuActionRow] = 561 + c;
					}
					if (l == 1) {
						menuActionIDs[menuActionRow] = 779 + c;
					}
					if (l == 2) {
						menuActionIDs[menuActionRow] = 27 + c;
					}
					if (l == 3) {
						menuActionIDs[menuActionRow] = 577 + c;
					}
					if (l == 4) {
						menuActionIDs[menuActionRow] = 729 + c;
					}
					menuItemDefIDs[menuActionRow] = j;
					menuInventorySlots[menuActionRow] = i;
					menuInvInterfaceIDs[menuActionRow] = k;
		...
		for (int i1 = 0; i1 < menuActionRow; i1++) {
			if (menuActionIDs[i1] == 516) {
				menuActionNames[i1] = "Walk here @whi@" + s;
				return;
			}
		}
	...
	public final void method90() {
		...
		if (previousSong > 0) {
			...
			if (previousSong == 0 && musicVolume != 0 && currentSong != -1) {
				setNextSong2(musicVolume, false, currentSong);
			}
		}
	}
...
//		}
		...
		try {
			...
			mChatDrawingArea = new TextDrawingArea(false, "p12_full", titleStreamLoader);
			...
			for (int j = 0; j < 4; j++) {
				mCollisionMap[j] = new CollisionMap();
			}

			mSomekindOfSprite = new Sprite(512, 512);
...
//			musics();
			...
			for (int i1 = 0; i1 < k; i1++) {
				onDemandFetcher.getSongFromIndex(1, i1);
			}
			...
			for (int k1 = 0; k1 < k; k1++) {
				...
				if ((l1 & 1) != 0) {
					onDemandFetcher.getSongFromIndex(0, k1);
				}
			}
			...
			if (decompressors[0] != null) {
				...
				onDemandFetcher.getSongFromIndex(3, onDemandFetcher.method562(0, 48, 47));
				onDemandFetcher.getSongFromIndex(3, onDemandFetcher.method562(1, 48, 47));
				onDemandFetcher.getSongFromIndex(3, onDemandFetcher.method562(0, 48, 48));
				onDemandFetcher.getSongFromIndex(3, onDemandFetcher.method562(1, 48, 48));
				onDemandFetcher.getSongFromIndex(3, onDemandFetcher.method562(0, 48, 49));
				onDemandFetcher.getSongFromIndex(3, onDemandFetcher.method562(1, 48, 49));
				onDemandFetcher.getSongFromIndex(3, onDemandFetcher.method562(0, 47, 47));
				onDemandFetcher.getSongFromIndex(3, onDemandFetcher.method562(1, 47, 47));
				onDemandFetcher.getSongFromIndex(3, onDemandFetcher.method562(0, 47, 48));
				onDemandFetcher.getSongFromIndex(3, onDemandFetcher.method562(1, 47, 48));
				onDemandFetcher.getSongFromIndex(3, onDemandFetcher.method562(0, 148, 48));
				onDemandFetcher.getSongFromIndex(3, onDemandFetcher.method562(1, 148, 48));
				k = onDemandFetcher.getNodeCount();
				while (onDemandFetcher.getNodeCount() > 0) {
			...
			TextDrawingArea aclass30_sub2_sub1_sub4s[] = {aTextDrawingArea_1270, mChatDrawingArea, chatTextDrawingArea, aTextDrawingArea_1273};
			...
			startRunnableThread(mouseDetection, 10);
	...
	public void method91(Stream stream, int i) {
		while (stream.bitPosition + 10 < i * 8) {
			...
			if (mPlayerArray[j] == null) {
				mPlayerArray[j] = new Player();
				if (aStreamArray895s[j] != null) {
					mPlayerArray[j].updatePlayer(aStreamArray895s[j]);
				}
			}
			mPlayerIndices[mPlayerCount++] = j;
			Player player = mPlayerArray[j];
	...
	public void processMainScreenClick() {
		...
		if (super.clickMode3 == 1) {
			...
			if (i >= 0 && j >= 0 && i < 146 && j < 151) {
				...
				if (flag1) {
					mStream.writeWordBigEndian(i);
					mStream.writeWordBigEndian(j);
					mStream.writeWord(minimapInt1);
					mStream.writeWordBigEndian(57);
					mStream.writeWordBigEndian(minimapInt2);
					mStream.writeWordBigEndian(minimapInt3);
					mStream.writeWordBigEndian(89);
					mStream.writeWord(myPlayer.x);
					mStream.writeWord(myPlayer.y);
					mStream.writeWordBigEndian(anInt1264);
					mStream.writeWordBigEndian(63);
				}
			}
			anInt1117++;
			if (anInt1117 > 1151) {
				anInt1117 = 0;
				mStream.createFrame(246);
				mStream.writeWordBigEndian(0);
				int l = mStream.currentOffset;
				if ((int) (Math.random() * 2D) == 0) {
					mStream.writeWordBigEndian(101);
				}
				mStream.writeWordBigEndian(197);
				mStream.writeWord((int) (Math.random() * 65536D));
				mStream.writeWordBigEndian((int) (Math.random() * 256D));
				mStream.writeWordBigEndian(67);
				mStream.writeWord(14214);
				if ((int) (Math.random() * 2D) == 0) {
					mStream.writeWord(29487);
				}
				mStream.writeWord((int) (Math.random() * 65536D));
				if ((int) (Math.random() * 2D) == 0) {
					mStream.writeWordBigEndian(220);
				}
				mStream.writeWordBigEndian(180);
				mStream.writeBytes(mStream.currentOffset - l);
			}
		}
	}
	...
	public void drawGameScreen() {
		if (fullScreenBackDialogID != -1 && (mLoadingStage == 2 || super.fullGameScreen != null)) {
			if (mLoadingStage == 2) {
		...
		if (aBoolean1233) {
			...
			mChatDrawingArea.method382(0xffffff, 55, "Public chat", 28, true);
			if (publicChatMode == 0) {
				mChatDrawingArea.method382(65280, 55, "On", 41, true);
			}
			if (publicChatMode == 1) {
				mChatDrawingArea.method382(0xffff00, 55, "Friends", 41, true);
			}
			if (publicChatMode == 2) {
				mChatDrawingArea.method382(0xff0000, 55, "Off", 41, true);
			}
			if (publicChatMode == 3) {
				mChatDrawingArea.method382(65535, 55, "Hide", 41, true);
			}
			mChatDrawingArea.method382(0xffffff, 184, "Private chat", 28, true);
			if (privateChatMode == 0) {
				mChatDrawingArea.method382(65280, 184, "On", 41, true);
			}
			if (privateChatMode == 1) {
				mChatDrawingArea.method382(0xffff00, 184, "Friends", 41, true);
			}
			if (privateChatMode == 2) {
				mChatDrawingArea.method382(0xff0000, 184, "Off", 41, true);
			}
			mChatDrawingArea.method382(0xffffff, 324, "Trade/compete", 28, true);
			if (tradeMode == 0) {
				mChatDrawingArea.method382(65280, 324, "On", 41, true);
			}
			if (tradeMode == 1) {
				mChatDrawingArea.method382(0xffff00, 324, "Friends", 41, true);
			}
			if (tradeMode == 2) {
				mChatDrawingArea.method382(0xff0000, 324, "Off", 41, true);
			}
			mChatDrawingArea.method382(0xffffff, 458, "Report abuse", 33, true);
	...
	public boolean buildFriendsListMenu(RSInterface class9) {
		...
		if (i >= 1 && i <= 200 || i >= 701 && i <= 900) {
			...
			menuActionNames[menuActionRow] = "Remove @whi@" + mFriendsList[i];
			menuActionIDs[menuActionRow] = 792;
			menuActionRow++;
			menuActionNames[menuActionRow] = "Message @whi@" + mFriendsList[i];
			menuActionIDs[menuActionRow] = 639;
			menuActionRow++;
			return true;
		}
		if (i >= 401 && i <= 500) {
			menuActionNames[menuActionRow] = "Remove @whi@" + class9.disabledText;
	...
	public void method104() {
		Animable_Sub3 class30_sub2_sub4_sub3 = (Animable_Sub3) mNodeList1.reverseGetFirst();
		for (; class30_sub2_sub4_sub3 != null; class30_sub2_sub4_sub3 = (Animable_Sub3) mNodeList1.reverseGetNext()) {
			...
			if (component.type == 0) {
				...
				if (component.type == 2) {
					...
					for (int l3 = 0; l3 < component.height; l3++) {
						for (int l4 = 0; l4 < component.width; l4++) {
							...
							if (component.inv[i3] > 0) {
								...
								if (k5 > DrawingArea.topX - 32 && k5 < DrawingArea.bottomX && j6 > DrawingArea.topY - 32 && j6 < DrawingArea.bottomY || activeInterfaceType != 0 && mLastMenuAction2 == i3) {
									int l9 = 0;
									...
									if (class30_sub2_sub1_sub1_2 != null) {
										if (activeInterfaceType != 0 && mLastMenuAction2 == i3 && lastMenuAction3 == component.id) {
											k6 = super.mouseX - lastSaveClickX;
											j7 = super.mouseY - lastSaveClickY;
											if (k6 < 5 && k6 > -5) {
												k6 = 0;
											}
											if (j7 < 5 && j7 > -5) {
										...
										if (class30_sub2_sub1_sub1_2.trimWidth == 33 || component.invStackSizes[i3] != 1) {
											...
											aTextDrawingArea_1270.drawTextMessage(0, intToKOrMil(k10), j6 + 10 + j7, k5 + 1 + k6);
											aTextDrawingArea_1270.drawTextMessage(0xffff00, intToKOrMil(k10), j6 + 9 + j7, k5 + k6);
										}
									}
								}
					...
					if (interfaceIsSelected(component)) {
						j3 = component.anInt219;
						if (flag && component.anInt239 != 0) {
							j3 = component.anInt239;
						}
					} else {
						j3 = component.textColor;
						if (flag && component.anInt216 != 0) {
							j3 = component.anInt216;
						}
					}
					...
					for (int l6 = l2 + textDrawingArea.anInt1497; s.length() > 0; l6 += textDrawingArea.anInt1497) {
						...
						if (component.aBoolean223) {
							textDrawingArea.method382(i4, k2 + component.width / 2, s1, l6, component.aBoolean268);
						} else {
							textDrawingArea.method389(component.aBoolean268, k2, i4, s1, l6);
						}
					}
					...
					for (int j5 = 0; j5 < component.height; j5++) {
						for (int i6 = 0; i6 < component.width; i6++) {
							if (component.inv[k4] > 0) {
								...
								if (component.aBoolean223) {
									textDrawingArea_1.method382(component.textColor, i9 + component.width / 2, s2, k9, component.aBoolean268);
								} else {
									textDrawingArea_1.method389(component.aBoolean268, i9, component.textColor, s2, k9);
								}
							}
							k4++;
						}
	...
	public void randomizeBackground(Background background) {
		int j = 256;
		for (int k = 0; k < mBuffer5.length; k++) {
			mBuffer5[k] = 0;
		}

		for (int l = 0; l < 5000; l++) {
			int i1 = (int) (Math.random() * 128D * (double) j);
			mBuffer5[i1] = (int) (Math.random() * 256D);
		}
		...
		for (int j1 = 0; j1 < 20; j1++) {
			for (int k1 = 1; k1 < j - 1; k1++) {
				for (int i2 = 1; i2 < 127; i2++) {
					int k2 = i2 + (k1 << 7);
					mBuffer6[k2] = (mBuffer5[k2 - 1] + mBuffer5[k2 + 1] + mBuffer5[k2 - 128] + mBuffer5[k2 + 128]) / 4;
				}
	...
	public void method108() {
		try {
			...
			if (anInt1005 > 1512) {
				anInt1005 = 0;
				mStream.createFrame(77);
				mStream.writeWordBigEndian(0);
				int i2 = mStream.currentOffset;
				mStream.writeWordBigEndian((int) (Math.random() * 256D));
				mStream.writeWordBigEndian(101);
				mStream.writeWordBigEndian(233);
				mStream.writeWord(45092);
				if ((int) (Math.random() * 2D) == 0) {
					mStream.writeWord(35784);
				}
				mStream.writeWordBigEndian((int) (Math.random() * 256D));
				mStream.writeWordBigEndian(64);
				mStream.writeWordBigEndian(38);
				mStream.writeWord((int) (Math.random() * 65536D));
				mStream.writeWord((int) (Math.random() * 65536D));
				mStream.writeBytes(mStream.currentOffset - i2);
			}
			int j2 = k1 * 192;
			if (j2 > 0x17f00) {
				j2 = 0x17f00;
	...
	public boolean isFriendOrSelf(String s) {
		...
		for (int i = 0; i < mFriendsCount; i++) {
			if (s.equalsIgnoreCase(mFriendsList[i])) {
				return true;
			}
		}
	...
	public void draw3dScreen() {
		...
		if (fpsOn) {
			...
			mChatDrawingArea.method380("Fps:" + super.fps, c, i1, k);
			k += 15;
			Runtime runtime = Runtime.getRuntime();
			int j1 = (int) ((runtime.totalMemory() - runtime.freeMemory()) / 1024L);
			i1 = 0xffff00;
			if (j1 > 0x2000000 && mLowMem) {
				i1 = 0xff0000;
			}
			mChatDrawingArea.method380("Mem:" + j1 + "k", c, 0xffff00, k);
			k += 15;
		}
		if (anInt1104 != 0) {
			int j = anInt1104 / 50;
			...
			j %= 60;
			if (j < 10) {
				mChatDrawingArea.drawTextMessage(0xffff00, "System update in: " + l + ":0" + j, 329, 4);
			} else {
				mChatDrawingArea.drawTextMessage(0xffff00, "System update in: " + l + ":" + j, 329, 4);
			}
			anInt849++;
			if (anInt849 > 75) {
				anInt849 = 0;
				mStream.createFrame(148);
			}
		}
	}
	...
	public void addIgnore(long l) {
		try {
			...
			for (int k = 0; k < mFriendsCount; k++) {
				if (mFriendsListAsLongs[k] == l) {
			...
			mNeedDrawTabArea = true;
			mStream.createFrame(133);
			mStream.writeQWord(l);
			return;
		} catch (RuntimeException runtimeexception) {
	...
	public void determineMenuSize() {
		...
		for (int j = 0; j < menuActionRow; j++) {
			int k = chatTextDrawingArea.getTextWidth(menuActionNames[j]);
			if (k > i) {
				i = k;
			}
		}
		...
		if (super.saveClickX > 4 && super.saveClickY > 4 && super.saveClickX < 516 && super.saveClickY < 338) {
			...
			mMenuOffsetY = l1;
			mMenuWidth = i;
			mMenuHeight = 15 * menuActionRow + 22;
		}
	...
	public void method117(Stream stream) {
		...
		if (k == 0) {
			anIntArray894[anInt893++] = mMyPlayerIndex;
			return;
		}
		if (k == 1) {
	...
	public void nullLoader() {
		...
		mTitleboxBackground = null;
		mTitlebuttonBackground = null;
		mRuneBackgrounds = null;
		mBuffer4 = null;
		mBuffer1 = null;
		mBuffer2 = null;
		mBuffer3 = null;
	...
	public void drawTooltip() {
		...
		if (itemSelected == 1 && menuActionRow < 2) {
			...
			s = menuActionNames[menuActionRow - 1];
		}
		if (menuActionRow > 2) {
	...
	public void drawMinimap() {
		...
		mSomekindOfSprite.method352(151, i, anIntArray1229, 256 + minimapInt3, anIntArray1052, l2, 5, 25, 146, j);
		compass.method352(33, minimapInt1, anIntArray1057, 256, anIntArray968, 25, 0, 0, 33, 25);
		...
		for (int i6 = 0; i6 < mNumNpcs; i6++) {
			NPC npc = npcArray[mNpcIndices[i6]];
		...
		for (int j6 = 0; j6 < mPlayerCount; j6++) {
			Player player = mPlayerArray[mPlayerIndices[j6]];
			if (player != null && player.isVisible()) {
				int j1 = player.x / 32 - myPlayer.x / 32;
				int l3 = player.y / 32 - myPlayer.y / 32;
				boolean flag1 = false;
				long l6 = TextClass.longForName(player.name);
				for (int k6 = 0; k6 < mFriendsCount; k6++) {
					if (l6 != mFriendsListAsLongs[k6] || mFriendsNodeIDs[k6] == 0) {
						continue;
					}
					flag1 = true;
					break;
		...
		if (anInt855 != 0 && loopCycle % 20 < 10) {
			...
			if (anInt855 == 10 && anInt933 >= 0 && anInt933 < mPlayerArray.length) {
				Player class30_sub2_sub4_sub1_sub2_1 = mPlayerArray[anInt933];
				if (class30_sub2_sub4_sub1_sub2_1 != null) {
			...
			if (chatMessages[j] != null) {
				...
				if ((k == 3 || k == 7) && (k == 7 || privateChatMode == 0 || privateChatMode == 1 && isFriendOrSelf(s))) {
					...
					if (super.mouseX > 4 && super.mouseY - 4 > l - 10 && super.mouseY - 4 <= l + 3) {
						int i1 = mChatDrawingArea.getTextWidth("From:  " + s + chatMessages[j]) + 25;
						if (i1 > 450) {
							i1 = 450;
						}
						if (super.mouseX < 4 + i1) {
							...
							menuActionRow++;
							menuActionNames[menuActionRow] = "Reply to @whi@" + s;
							menuActionIDs[menuActionRow] = 2639;
							menuActionRow++;
							menuActionNames[menuActionRow] = "Add friend @whi@" + s;
	...
	public void doFlamesDrawing() {
		...
		if (anInt1040 > 0) {
			for (int i = 0; i < 256; i++) {
				if (anInt1040 > 768) {
					mBuffer4[i] = method83(mBuffer1[i], mBuffer2[i], 1024 - anInt1040);
				} else if (anInt1040 > 256) {
					mBuffer4[i] = mBuffer2[i];
				} else {
					mBuffer4[i] = method83(mBuffer2[i], mBuffer1[i], 256 - anInt1040);
				}
			}
			...
			for (int j = 0; j < 256; j++) {
				if (anInt1041 > 768) {
					mBuffer4[j] = method83(mBuffer1[j], mBuffer3[j], 1024 - anInt1041);
				} else if (anInt1041 > 256) {
					mBuffer4[j] = mBuffer3[j];
				} else {
					mBuffer4[j] = method83(mBuffer3[j], mBuffer1[j], 256 - anInt1041);
				}
			}

		} else {
			System.arraycopy(mBuffer1, 0, mBuffer4, 0, 256);

		}
		System.arraycopy(mSprite1.pixels, 0, mSprite1ImageProducer.pixels, 0, 33920);
		...
		for (int k1 = 1; k1 < c - 1; k1++) {
			...
			for (int l2 = j2; l2 < 128; l2++) {
				...
				if (j3 != 0) {
					...
					int j4 = 256 - j3;
					j3 = mBuffer4[j3];
					int l4 = mSprite1ImageProducer.pixels[j1];
					mSprite1ImageProducer.pixels[j1++] = ((j3 & 0xff00ff) * l3 + (l4 & 0xff00ff) * j4 & 0xff00ff00) + ((j3 & 0xff00) * l3 + (l4 & 0xff00) * j4 & 0xff0000) >> 8;
				} else {
					j1++;
				}
			}
		...
		mSprite1ImageProducer.drawGraphics(0, super.graphics, 0);
		System.arraycopy(mSprite2.pixels, 0, mSprite2ImageProducer.pixels, 0, 33920);
		...
		for (int k2 = 1; k2 < c - 1; k2++) {
			...
			for (int i4 = 0; i4 < k3; i4++) {
				...
				if (k4 != 0) {
					...
					int j5 = 256 - k4;
					k4 = mBuffer4[k4];
					int k5 = mSprite2ImageProducer.pixels[j1];
					mSprite2ImageProducer.pixels[j1++] = ((k4 & 0xff00ff) * i5 + (k5 & 0xff00ff) * j5 & 0xff00ff00) + ((k4 & 0xff00) * i5 + (k5 & 0xff00) * j5 & 0xff0000) >> 8;
				} else {
					j1++;
				}
			}
	...
	public void method134(Stream stream) {
		...
		if (j < mPlayerCount) {
			for (int k = j; k < mPlayerCount; k++) {
				anIntArray840[anInt839++] = mPlayerIndices[k];
			}
		...
		mPlayerCount = 0;
		for (int l = 0; l < j; l++) {
			int i1 = mPlayerIndices[l];
			Player player = mPlayerArray[i1];
			int j1 = stream.readBits(1);
			if (j1 == 0) {
				mPlayerIndices[mPlayerCount++] = i1;
				player.loopCycle = loopCycle;
			} else {
	...
	public void drawLoginScreen(boolean flag) {
		resetImageProducers();
		aRSImageProducer_1109.initDrawingArea();
		mTitleboxBackground.method361(0, 0);
		char c = '\u0168';
		char c1 = '\310';
		if (loginScreenState == 0) {
			...
			int l = c / 2 - 80;
			int k1 = c1 / 2 + 20;
			mTitlebuttonBackground.method361(l - 73, k1 - 20);
		...
		if (loginScreenState == 2) {
			...
			if (!flag) {
				int i1 = c / 2 - 80;
				int l1 = c1 / 2 + 50;
				mTitlebuttonBackground.method361(i1 - 73, l1 - 20);
		...
		if (loginScreenState == 3) {
			...
			int j1 = c / 2;
			int i2 = c1 / 2 + 50;
			mTitlebuttonBackground.method361(j1 - 73, i2 - 20);
	...
	public void method137(Stream stream, int j) {
		...
		if (j == 105) {
			...
			int i14 = l11 >> 4 & 0xf;
			int i16 = l11 & 7;
			if (myPlayer.smallX[0] >= k3 - i14 && myPlayer.smallX[0] <= k3 + i14 && myPlayer.smallY[0] >= j6 - i14 && myPlayer.smallY[0] <= j6 + i14 && aBoolean848 && !mLowMem && currentSound < 50) {
		...
		if (j == 4) {
			...
			if (i5 >= 0 && l7 >= 0 && i5 < 104 && l7 < 104) {
				...
				mNodeList1.insertHead(class30_sub2_sub4_sub3);
			}
			return;
		}
		if (j == 44) {
		...
		if (j == 117) {
			...
			if (l5 >= 0 && k8 >= 0 && l5 < 104 && k8 < 104 && j11 >= 0 && k13 >= 0 && j11 < 104 && k13 < 104 && i17 != 65535) {
				...
				mNodeList2.insertHead(class30_sub2_sub4_sub4);
			}
		}
	}
	...
	public void method139(Stream stream) {
		...
		if (k < mNumNpcs) {
			for (int l = k; l < mNumNpcs; l++) {
				anIntArray840[anInt839++] = mNpcIndices[l];
			}
		...
		mNumNpcs = 0;
		for (int i1 = 0; i1 < k; i1++) {
			int j1 = mNpcIndices[i1];
			NPC npc = npcArray[j1];
			int k1 = stream.readBits(1);
			if (k1 == 0) {
				mNpcIndices[mNumNpcs++] = j1;
				npc.loopCycle = loopCycle;
			} else {
			...
			if (i2 != 0) {
				...
				if (j1 == 0) {
					...
					if (class46.aBoolean767) {
						mCollisionMap[j].method215(l2, k2, class46.aBoolean757, i1, i);
					}
				}
				if (j1 == 1) {
				...
				if (j1 == 2) {
					...
					if (class46_1.aBoolean767) {
						mCollisionMap[j].method216(l2, class46_1.anInt744, i1, i, class46_1.anInt761, class46_1.aBoolean757);
					}
				}
				if (j1 == 3) {
					...
					if (class46_2.aBoolean767 && class46_2.hasActions) {
						mCollisionMap[j].method218(i, i1);
					}
				}
			}
			if (k1 >= 0) {
				...
				ObjectManager.method188(worldController, k, i, l, j3, mCollisionMap[j], intGroundArray, i1, k1, j);
			}
		}
	}
	...
	public void updatePlayers(int i, Stream stream) {
		...
		for (int k = 0; k < anInt839; k++) {
			...
			if (mPlayerArray[l].loopCycle != loopCycle) {
				mPlayerArray[l] = null;
			}
		}
		...
		for (int i1 = 0; i1 < mPlayerCount; i1++) {
			if (mPlayerArray[mPlayerIndices[i1]] == null) {
				Signlink.reporterror(myUsername + " null entry in pl list - pos:" + i1 + " size:" + mPlayerCount);
				throw new RuntimeException("eek");
			}
		}
		...
		try {
			...
			if (pktType == 74) {
				...
				if (i2 != -1 || previousSong != 0) {
					if (i2 != -1 && currentSong != i2 && musicVolume != 0 && previousSong == 0)
						setNextSong(10, musicVolume, false, i2);
				} else
					method55(false);
				currentSong = i2;
				pktType = -1;
			...
			if (pktType == 121) {
				...
				if (musicVolume != 0 && i_61_ != -1) {
					setNextSong2(musicVolume, false, i_60_);
			...
			if (pktType == 73 || pktType == 241) {
				...
				if (anInt1069 == l2 && anInt1070 == i11 && mLoadingStage == 2) {
					pktType = -1;
					return true;
				}
				anInt1069 = l2;
					...
					for (int l23 = (anInt1069 - 6) / 8; l23 <= (anInt1069 + 6) / 8; l23++) {
						for (int j26 = (anInt1070 - 6) / 8; j26 <= (anInt1070 + 6) / 8; j26++) {
							...
							if (aBoolean1141 && (j26 == 49 || j26 == 149 || j26 == 147 || l23 == 50 || l23 == 49 && j26 == 47)) {
								...
								if (k28 != -1) {
									onDemandFetcher.getSongFromIndex(3, k28);
								}
								int j30 = anIntArray1236[k16] = onDemandFetcher.method562(1, j26, l23);
								if (j30 != -1) {
									onDemandFetcher.getSongFromIndex(3, j30);
								}
								k16++;
							}
						}
				...
				if (pktType == 241) {
					...
					for (int l26 = 0; l26 < l16; l26++) {
						...
						if (j32 != -1) {
							onDemandFetcher.getSongFromIndex(3, j32);
						}
						int i33 = anIntArray1236[l26] = onDemandFetcher.method562(1, l31, l30);
						if (i33 != -1) {
							onDemandFetcher.getSongFromIndex(3, i33);
						}
					}
				...
				for (int i27 = 0; i27 < maxPlayers; i27++) {
					Player player = mPlayerArray[i27];
					if (player != null) {
			...
			if (pktType == 1) {
				for (int k4 = 0; k4 < mPlayerArray.length; k4++) {
					if (mPlayerArray[k4] != null) {
						mPlayerArray[k4].anim = -1;
					}
				}
			...
			if (pktType == 50) {
				...
				for (int k24 = 0; k24 < mFriendsCount; k24++) {
					if (l4 != mFriendsListAsLongs[k24]) {
						continue;
					}
					if (mFriendsNodeIDs[k24] != i18) {
						mFriendsNodeIDs[k24] = i18;
						mNeedDrawTabArea = true;
						if (i18 >= 2) {
				...
				if (s7 != null && mFriendsCount < 200) {
					mFriendsListAsLongs[mFriendsCount] = l4;
					mFriendsList[mFriendsCount] = s7;
					mFriendsNodeIDs[mFriendsCount] = i18;
					mFriendsCount++;
					mNeedDrawTabArea = true;
				}
				for (boolean flag6 = false; !flag6;) {
					flag6 = true;
					for (int k29 = 0; k29 < mFriendsCount - 1; k29++) {
						if (mFriendsNodeIDs[k29] != nodeID && mFriendsNodeIDs[k29 + 1] == nodeID || mFriendsNodeIDs[k29] == 0 && mFriendsNodeIDs[k29 + 1] != 0) {
							int j31 = mFriendsNodeIDs[k29];
							mFriendsNodeIDs[k29] = mFriendsNodeIDs[k29 + 1];
							mFriendsNodeIDs[k29 + 1] = j31;
							String s10 = mFriendsList[k29];
							mFriendsList[k29] = mFriendsList[k29 + 1];
							mFriendsList[k29 + 1] = s10;
							long l32 = mFriendsListAsLongs[k29];
							mFriendsListAsLongs[k29] = mFriendsListAsLongs[k29 + 1];
							mFriendsListAsLongs[k29 + 1] = l32;
							mNeedDrawTabArea = true;
							flag6 = false;
						}
					}
			...
			if (pktType == 249) {
				mMemberStatus = inStream.readUnsignedByteA();
	...
	public Game() {
		...
		maxPlayers = 2048;
		mMyPlayerIndex = 2047;
		mPlayerArray = new Player[maxPlayers];
		mPlayerIndices = new int[maxPlayers];
		...
		aBoolean954 = true;
		mFriendsListAsLongs = new long[200];
		currentSong = -1;
		drawingFlames = false;
		spriteDrawX = -1;
		spriteDrawY = -1;
		...
		aBoolean1080 = false;
		mFriendsList = new String[200];
		inStream = Stream.create();
		expectedCRCs = new int[9];
		menuInventorySlots = new int[500];
		menuInvInterfaceIDs = new int[500];
		menuActionIDs = new int[500];
		menuItemDefIDs = new int[500];
		...
		anInt1184 = 128;
		invOverlayInterfaceID = -1;
		mStream = Stream.create();
		menuActionNames = new String[500];
		...
		mCollisionMap = new CollisionMap[4];
		aBoolean1233 = false;
	...
	public long mCurrentTimeMilli;
	public int[][] anIntArrayArray825;
	public int[] mFriendsNodeIDs;
	...
	public final int mMyPlayerIndex;
	public Player[] mPlayerArray;
	public int mPlayerCount;
	public int[] mPlayerIndices;
	public int anInt893;
	...
	public int mMenuOffsetX;
	public int mMenuOffsetY;
	public int mMenuWidth;
	public int mMenuHeight;
	public long aLong953;
	public boolean aBoolean954;
	public long[] mFriendsListAsLongs;
	public int currentSong;
	public static int nodeID = 10;
	static int portOff;
	public static boolean isMembers = true;
	public static boolean mLowMem;
	...
	public Background mTitleboxBackground;
	public Background mTitlebuttonBackground;
	...
	public int mMouseInvInterfaceIndex;
	...
	public String[] mFriendsList;
	public Stream inStream;
	public int lastMenuAction3;
	public int mLastMenuAction2;
	public int activeInterfaceType;
	public int lastSaveClickX;
	public int lastSaveClickY;
	public int anInt1089;
	public final int[] expectedCRCs;
	public int[] menuInventorySlots;
	public int[] menuInvInterfaceIDs;
	public int[] menuActionIDs;
	public int[] menuItemDefIDs;
	...
	public RSImageProducer mSprite1ImageProducer;
	public RSImageProducer mSprite2ImageProducer;
	...
	public Background[] mRuneBackgrounds;
	public boolean mNeedDrawTabArea;
	...
	public String[] menuActionNames;
	public Sprite mSprite1;
	...
	public CollisionMap[] mCollisionMap;
	...
	public int destX;
	public int destY;
	public Sprite mSomekindOfSprite;
	...
	public TextDrawingArea mChatDrawingArea;
	...
	public static SoundClass music;
